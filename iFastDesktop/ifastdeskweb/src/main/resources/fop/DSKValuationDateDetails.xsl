...
<xsl:variable name="response" select="//Valn"/>
		...
		<table border="1">
			...
			<thead border="1">
				<tr>
					...
					<th width="90px">Benchmark <br/> Adj. Rate</th>
					<th width="70px">Weighting</th>
					<th width="90px">Period</th>
					<th width="50px">Number <br/> of Days</th>
					<th width="80px">Day Basis</th>
				</tr>
			</thead>
			<xsl:if test="//Valn/Hurdles/Hurdle">
				<tbody border="1">
					<xsl:for-each select="//Valn/Hurdles/Hurdle">
						<xsl:call-template  name="tmpHurdleDetailsRecord">
						</xsl:call-template>
					</xsl:for-each>
				</tbody>
			</xsl:if>
			<xsl:if test="not(//Valn/Hurdles/Hurdle)">
				<tbody border="0">
					<xsl:call-template  name="tmpHurdleDetailsRecord">
					</xsl:call-template>
				</tbody>
	...
	<xsl:template name="tmpHurdleDetailsRecord">
						<tr>
							<xsl:choose>
