 ...
 *  		Remove a session-id parameter from url in buildServiceCallURL function.
...
DesktopWeb.Ajax = {
	...
	buildURL: function(params)
	{
		...
		{
			var param = params[i];
			// Temp checks to leave functionality for un-converted controllers in the DesktopWebController
			if (i.toLowerCase() === "jobname") {
				if(param !== "CallService")
					baseUrl = param.charAt(0).toLowerCase() + param.slice(1) + '?';
				else
					baseUrl = 'smartViewAjaxService?';
			} else {
				url += "&" + i + "=" + param;
			}
		}
	...
	,buildDeleteCacheRequestXML: function(cacheKeys)
	{
		var xml = IFDS.Xml.newDocument("DeleteCacheRequest");
	...
	,deleteCacheRequest: function(cacheKeys, callback)
	{
		Ext.Ajax.request(
			{
				url: 'deleteCache'				
				...
				,method: 'POST'
				,timeout: 90000
				,xmlData: this.buildDeleteCacheRequestXML(cacheKeys)
			}	
		);
	}
	...
	{
		var xml = IFDS.Xml.newDocument("request");
		
		var apiHeader = IFDS.Xml.addSingleNode(xml, smartview +'Request');
		
		var header = IFDS.Xml.addSingleNode(apiHeader, 'RequestHeader');
		...
		var body = IFDS.Xml.addSingleNode(apiHeader, 'data');
