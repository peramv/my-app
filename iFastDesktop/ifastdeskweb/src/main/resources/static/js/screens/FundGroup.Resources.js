 ...
 *                  - fixing re-populate fund/class/isin/wkn
 *
 *  17 Nov 2016 Winnie Cheng. P0256381  T84279 DFT0069397 CHG0045765 Fund Broker Linkage for Germany
 *                  - remove comment
 *
 *  19 Dec 2016 Winnie Cheng. P0256381  T84531 DFT0070493 CHG0045765 Fund Broker Linkage for Germany
 *          - Add fund class popUp - search,ensure displaying as fund + class + fundName format only
 */
...
DesktopWeb.ScreenResources = function(ctrlr)
{
			...
			{							
				...
				,colModel: new Ext.grid.ColumnModel({
					...
					,columns: [{
						...
						,{header: _translationMap['GroupDesc'], id: 'grpDesc', dataIndex: 'grpDesc', width: 547}
					]
				})
				,buttons: [
		...
		,linkedFundClass: new DesktopWeb.AdvancedControls.GridPanel({
			...
			,colModel: new Ext.grid.ColumnModel({
				...
				,columns: [{
					...
					,{header: _translationMap['Fund'], dataIndex: 'fund', width: 70}
					,{header: _translationMap['Class'], dataIndex: 'class', width: 50}
					,{header: _translationMap['FundName'], id: 'fundName', dataIndex: 'fundName', width: 290}
					,{header: _translationMap['ISIN'], dataIndex: 'isin', width: 130} 
					,{header: _translationMap['WKN'], dataIndex: 'wkn', width: 110}
				]							
			})
			,buttons: [
				new DesktopWeb.Controls.ActionButton({
	...
	var _popups = {
		...
		,linkedFundPopup: new DesktopWeb.AdvancedControls.PopupWindow({
				...
				,{
					...
					,items: [ 
						...
						,new DesktopWeb.Controls.SuggestField({
							...
							,smartview: 'fundSearch'				
							,fields: ['displayValue', 'fund', 'class', 'isin', 'wkn','fundName']
							,valueField: 'displayValue'		
							,displayField: 'displayValue'
							,width: 470
							,allowBlank: false
							,listeners: {
								populated: function(fld, rec){
									...
									if (rec.data['fund'].length > 0) {
										...
										var dispFund = rec.data['fund'] + ' ' + rec.data['class'] + ' ' + rec.data['fundName']
										popup.getField('fundclass').setValue(dispFund);
									}
								} 
								,cleared: function(){
									var popup = _popups['linkedFundPopup'];
			...
			,populate: function(action) {
				...
				switch(action) {
					...
					case _controller.DEL: {
						...
						popup.getField('fundclass').setRawValue(selectedlinkedFundClass.data['fund']);
			...
			,getData: function() {
				...
				//rawValue maybe fund/class/Name or fund only
				var rawValue = popup.getField('fundclass').getValue();
				//parse fund code
				var endOfFundPosition = rawValue.indexOf(' ');
				if (endOfFundPosition > 0 ) {
					data['fund'] = rawValue.substring(0, rawValue.indexOf(' '));
				} else {
					data['fund'] = rawValue;
				}
	...
	return {
		...
		,screenButtons: (DesktopWeb._SCREEN_PARAM_MAP['extLayout'] !== 'popup') ? [			
