...
bool MFDSTCSetup::init( bool bDirectConnect, const bool& performTesting, DString& errorMessage)
{
  // Start singletons
  ...
  if (amqConfig.brokerURLs.empty()) amqConfig.brokerURLs = smqConfig.brokerURLs;
  ...
  {
	  // format the failover transport URL if not explicit.
//      decaf::lang::String strBrokerURL(DStringA(amqConfig.brokerURLs).c_str());
//      if (!strBrokerURL.startsWith("failover")) {

	  
      if (amqConfig.brokerURLs.substr(0,8) != I_("failover")) {
	     amqConfig.brokerURLs = I_("failover:(") + amqConfig.brokerURLs + I_(")?maxReconnectAttempts=0&randomize=false");
      }
  }
  ...
  if (!amqConfig.iTimeToLive) amqConfig.iTimeToLive = 0;

  amqConfig.iMaxConnections = dstrAmqMaxConnections.convertToULong();
                                    ...
                                    DString &errorMessage)
{
  ...
  if (testResult == false)
  {
    ...
    testResult = ifsupp::testConnectivity(
		service, (service == ifastdbrkr::MessageService::ActiveMQ ? amqConfig : smqConfig), currentMessage);
  }
