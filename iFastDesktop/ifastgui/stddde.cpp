...
BOOL CDDEStringItem::Request(UINT wFmt, void** ppData, DWORD* pdwSize)
{
    ASSERT(wFmt == CF_TEXT);
    ASSERT(ppData);
    *ppData = (void*)(const char*)m_strData.GetBuffer ();
...
CDDEItem* CDDETopic::FindItem(const char* pszItem)
{
    ...
    while (pos) {
        ...
        if (pItem->m_strName.CompareNoCase((LPCTSTR)pszItem) == 0) return pItem;
    }
    return NULL;
}
...
BOOL CDDEConv::ConnectTo(LPCTSTR pszService, LPCTSTR pszTopic)
{
    ...
    if (!m_hConv) {
        m_pServer->Status(L"Failed to connect to %s|%s. Error %u",
...
BOOL CDDESystemItem_TopicList::Request(UINT wFmt, void** ppData, DWORD* pdwSize)
{
    ...
    *ppData = (void*)(const char*)strTopics.GetBuffer ();
...
BOOL CDDESystemItem_ItemList::Request(UINT wFmt, void** ppData, DWORD* pdwSize)
{
    ...
    *ppData = (void*)(const char*)strItems.GetBuffer ();
...
BOOL CDDESystemItem_FormatList::Request(UINT wFmt, void** ppData, DWORD* pdwSize)
{
    ...
    *ppData = (void*)(const char*)strFormats.GetBuffer ();
...
BOOL CDDEServerSystemTopic::Request(UINT wFmt, const char* pszItem,
                                    void** ppData, DWORD* pdwSize)
{
    m_pServer->Status(L"System topic request: %s", pszItem);
                        ...
                        DWORD* pdwDDEInst/* = NULL */)
{
    ...
    m_hszServiceName.Create(this, (LPCTSTR)m_strServiceName.GetBuffer ());
    ...
    m_SystemTopic.Create((const char*)CString (SZDDESYS_TOPIC).GetBuffer ());
    ...
    m_SystemItemTopics.Create((const char*)CString (SZDDESYS_ITEM_TOPICS).GetBuffer ());
    m_SystemTopic.AddItem(&m_SystemItemTopics);

    m_SystemItemSysItems.Create((const char*)CString (SZDDESYS_ITEM_SYSITEMS).GetBuffer ());
    m_SystemTopic.AddItem(&m_SystemItemSysItems);

    m_SystemItemItems.Create((const char*)CString (SZDDE_ITEM_ITEMLIST).GetBuffer ());
    m_SystemTopic.AddItem(&m_SystemItemItems);

    m_SystemItemFormats.Create((const char*)CString (SZDDESYS_ITEM_FORMATS).GetBuffer ());
                                            ...
                                            DWORD dwData2)
{
    ...
    ASSERT(pServ);
    pServ->Status(L"Callback %4.4XH", wType);
    ...
    case XTYP_CONNECT_CONFIRM:
        ...
        pServ->Status(L"Connect to %s", (const char*)pServ->StringFromHsz(hsz1).GetBuffer ());
    ...
    case XTYP_WILDCONNECT:
        ...
        || !::DdeCmpStringHandles(hsz2, pServ->m_hszServiceName)) {
            ...
            pServ->Status(L"Wild connect to %s", (const char*)pServ->StringFromHsz(hsz1).GetBuffer ());
...
HDDEDATA CDDEServer::DoWildConnect(HSZ hszTopic)
{
    ...
    if (hszTopic == NULL) {
        ...
        strTopic = StringFromHsz(hszTopic).GetBuffer ();
        CDDETopic* pTopic = FindTopic((const char*)strTopic.GetBuffer ());
        if(pTopic) {
            iTopics++;
        }
    }
    ...
    if (!iTopics) {
        Status(L"Wild connect to %s refused", (const char*)strTopic.GetBuffer ());
        return (HDDEDATA) NULL;
    }
    ...
    if (hszTopic == NULL) {
        ...
        while (pos) {
            ...
            pHszPair->hszSvc = ::DdeCreateStringHandle(m_dwDDEInstance,
                                                       (LPCWSTR)m_strServiceName.GetBuffer (),
                                                       CP_WINUNICODE);
            pHszPair->hszTopic = ::DdeCreateStringHandle(m_dwDDEInstance,
                                                         (LPCWSTR)pTopic->m_strName.GetBuffer (),
                                                         CP_WINUNICODE);
...
CDDETopic* CDDEServer::FindTopic(const char* pszTopic)
{
    ...
    while (pos) {
        CDDETopic* pTopic = m_TopicList.GetNext(pos);
        if (pTopic->m_strName.CompareNoCase((LPCTSTR)pszTopic) == 0) return pTopic;
    }
    return NULL;
}
                            ...
                            HDDEDATA *phReturnData)
{
    ...
    if (wType == XTYP_EXECUTE) {
        ...
        BOOL b = Exec((const char*)strTopic.GetBuffer (), pData, dwLength);
        ::DdeUnaccessData(hData);
    ...
    if (wType == XTYP_CONNECT) {
        ...
        if (!FindTopic((const char*)strTopic.GetBuffer ())) return FALSE; // unknown topic
    ...
    case XTYP_ADVSTART:
        ...
        if (!CanAdvise(wFmt, (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer ())) {

            Status(L"Can't advise on %s|%s", (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer ());
            return FALSE;
        }
    ...
    case XTYP_POKE:
        ...
        b = Poke(wFmt, (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer (), pData, dwLength);
        ::DdeUnaccessData(hData);
        ...
        if (!b) {
            ...
            Status(L"Poke %s|%s failed", (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer ()); 
            return FALSE;
    ...
    case XTYP_ADVDATA:
        ...
        pData = ::DdeAccessData(hData, &dwLength);
        b = AdviseData(wFmt, hConv, (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer (), pData, dwLength);
        ::DdeUnaccessData(hData);
        ...
        if (!b) {
            ...
            Status(L"AdviseData %s|%s failed", (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer ()); 
            *phReturnData = (HDDEDATA) DDE_FNOTPROCESSED;
    ...
    case XTYP_REQUEST:
        ...
        Status(L"Request %s|%s", (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer ()); 
        dwLength = 0;
        if (!Request(wFmt, (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer (), &pData, &dwLength)) {
            ...
            Status(L"Request %s|%s failed", (const char*)strTopic.GetBuffer (), (const char*)strItem.GetBuffer()); 
            *phReturnData = NULL;
            return FALSE;
...
void CDDEServer::PostAdvise(CDDETopic* pTopic, CDDEItem* pItem)
{
    ...
    ::DdePostAdvise(m_dwDDEInstance,
                    ::DdeCreateStringHandle(m_dwDDEInstance,
                                            pTopic->m_strName.GetBuffer (),
                                            CP_WINUNICODE),
                    ::DdeCreateStringHandle(m_dwDDEInstance,
                                            pItem->m_strName.GetBuffer (),
                                            CP_WINUNICODE));
...
CString GetFormatName(WORD wFmt)
{
    ...
    char buf[256];
    if (::GetClipboardFormatName(wFmt, (LPWSTR)buf, sizeof(buf))) {
        strName = buf;
    }
