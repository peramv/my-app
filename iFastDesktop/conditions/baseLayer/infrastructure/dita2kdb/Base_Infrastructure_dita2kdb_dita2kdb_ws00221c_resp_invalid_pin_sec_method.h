...
namespace CND
{
	class CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_pin_sec_method : public CConditionObject
	{
	public:
		CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_pin_sec_method() { }
		~CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_pin_sec_method() { }
	...
	public:
		...
		// Messages
		virtual DString GetMessage_EN_US() const { return DString(I_("Communication with host returned an error, RESP_INVALID_PIN_SEC_METHOD. CICS return code is %CICSError%, WS00221 return code is %error%, WS00221 return location is %loc%.")); }
		virtual DString GetMessage_FR_FR() const { return DString(I_("Communication with host returned an error, RESP_INVALID_PIN_SEC_METHOD. CICS return code is %CICSError%, WS00221 return code is %error%, WS00221 return location is %loc%.")); }
		virtual DString GetMessage_DE_DE() const { return DString(I_("Kommunikation mit Host ergab einen Fehler, RESP_INVALID_PIN_SEC_METHOD. CICS-Rückgabecode ist %CICSError%, WS00221-Rückgabecode ist %error%, WS00221-Rückgabeort ist %loc%.")); }
		...
		virtual DString GetMessage_FR_CA() const { return DString(I_("La communication avec l'hôte a renvoyé une erreur, RESP_INVALID_PIN_SEC_METHOD. Le code de renvoi CICS est %CICSError%, le code de renvoi WS00221 est %error%, la localisation de renvoi WS00221 est %loc%.")); }
