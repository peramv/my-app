...
namespace CND
{
	class CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_city_entered : public CConditionObject
	{
	public:
		CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_city_entered() { }
		~CBase_Infrastructure_dita2kdb_dita2kdb_ws00221c_resp_invalid_city_entered() { }
	...
	public:
		...
		virtual DString GetMessage_DE_DE() const { return DString(I_("Kommunikation mit Host ergab einen Fehler, RESP_INVALID_CITY_ENTERED. CICS-Rückgabecode ist %CICSError%, WS00221-Rückgabecode ist %error%, WS00221-Rückgabeort ist %loc%.")); }
		virtual DString GetMessage_ES_ES() const { return DString(I_("La comunicación con el host devolvión un error, RESP_INVALID_CITY_ENTERED. El código de retorno CICS es %CICSError%, el código de retorno WS00221 es %error%, la ubicación de retorno WS00221 es %loc%.")); }
		virtual DString GetMessage_FR_CA() const { return DString(I_("La communication avec l'hôte a renvoyé une erreur, RESP_INVALID_CITY_ENTERED. Le code de renvoi CICS est %CICSError%, le code de renvoi WS00221 est %error%, la localisation de renvoi WS00221 est %loc%.")); }
