...
// last revised $Date:   Oct 13 2010 08:07:28  $ $Revision:   1.1  $
...
class CRegistry {
    ...
    public:
        CRegistry( HKEY OpenKey = HKEY_LOCAL_MACHINE ) ;
...
class CRegVal {
    ...
    public:
        CRegVal() ;
        CRegVal( DWORD ValueToSet );
        CRegVal( LPCTSTR ValueToSet );
    ...
    public:         // functions for the world
        ...
        CRegVal& SetValue( LPCTSTR Value, BOOL Expand = FALSE ) ;
        
        // use the following function to set up a multiple string (i.e. string
        // array)
        CRegVal& SetValue( LPCTSTR Array [], const DWORD Size ) ;

        
        // the following function allows you to set up binary data of any
        // type and size.
        CRegVal& SetValue( const LPBYTE Data, const DWORD Size ) ;
