<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:include href="{$APPCFG:ApplicationPrivateDirectory}{$APPCFG:DataDirectory}{$APP:CLIENT}/screens/MasterLayout.xsl"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:call-template name="master_layout">
      <xsl:with-param name="pageTitle">Clone Account</xsl:with-param>
  ...
  <xsl:template name="form_callback">
      ...
      <script>
        ...
        {
		  <xsl:choose>
			  <xsl:when test=".//RequestStatus/status = 'Rejected'">
				alert("<xsl:value-of select=".//RequestStatus/Errors/Error/text"/>");  
			  </xsl:when>
			  <xsl:otherwise>
		  ...
		  document.getElementById("brokerBox").focus();
        ...
        function OnchangeAcctDesign()
        {
          ...
          if( acctDesign != "3")
          {
            ...
            //document.getElementById("cmbIntermCode").disabled = true;
            document.getElementById("cmbIntermCode").disable( true );
            document.getElementById("IntermAcctNbr").value = "";
            document.getElementById("IntermAcctNbr").disabled = true;
          }
          else
          {
            document.getElementById("cmbIntermCode").disable( false );
          }
        }
	  ...
	  <xsl:if test=".//RequestStatus/status = 'Rejected'">
	      <div style="position:absolute; top:0; left:0; width:800px; height:300px; background:red; filter:alpha(opacity=0); z-index:1000">
			  .
		  </div>		  
	  </xsl:if>
      ...
      <fieldset align="center" id="initial" style="postion:absolute; left:25; top:50; width:750px; border:1px double #CCCCCC">
        <legend><font color="black">New Account Parameters:</font></legend>
        ...
        <table>
          <tr>
            ...
            <td style="position:relative; left:0px;" >
              ...
              <xsl:attribute name="onfocus">if(_brokerSuggest != null){_brokerSuggest.init2( this, document.getElementById("branchBox"), document.getElementById("repBox"), document.getElementById("brokerName"), document.getElementById("branchName"), document.getElementById("repName"), document.getElementById("fldallowBrUpd").value);}</xsl:attribute>					
              </xsl:element>
            </td>
            <td colspan="3">
              <div id="brokerName">
            ...
            <td id="branchID2" style="position:relative; left:0px;">
              ...
              <xsl:if test="not(//DefaultValues/allowBrUpd = 'no')">
                <xsl:attribute name="value"><xsl:value-of select="//DefaultValues/branch"/></xsl:attribute>
                <xsl:attribute name="code"><xsl:value-of select="//DefaultValues/branch"/></xsl:attribute>
                <xsl:attribute name="onfocus">if(_branchSuggest != null){_branchSuggest.init2(document.getElementById("brokerBox"), this, document.getElementById("repBox"), document.getElementById("branchName"), document.getElementById("repName"));}</xsl:attribute>
              </xsl:if>
            ...
            <td id="repID2" type="text" value="" style="position:relative; top:0; left:0;">
              ...
              <xsl:attribute name="onfocus">if(_salesrepSuggest != null){_salesrepSuggest.init2(document.getElementById("brokerBox"), document.getElementById("branchBox"), this, document.getElementById("repName"), document.getElementById("fldCreateSaleRep"), document.getElementById("branchName"));}</xsl:attribute>					
              </xsl:element>
            </td>
            <td colspan="5">
              <div id="repName">
            ...
            <td style="position:relative; left:0px;">
              ...
              <input type="hidden" class="dstcombobox" formfield="formfield" name="cmbAcctDesign" id="cmbAcctDesign" data="xmlcmbAcctDesign"  tabindex = "4" width="160px" onchange="OnchangeAcctDesign()" />
                <xml id="xmlcmbAcctDesign">
                    <newdataset>
						<xsl:choose>
							<xsl:when test="//dtAcctAutoCopyInitResponse/RequestStatus/status = 'Rejected'">
								    <combobox>
                                		<display></display>
										<formfield></formfield>
									</combobox>							
    ...
    <div align="right" style="position:relative; width:750px;">
      <!--
        - Using a button with an 'id' of btnOK will perform a check to see if the confirmation
        - dialog should be displayed.   It will call back into your JavaScript and look for the
        - 'LaunchConfirm()' method.  This method should return 'true' if you want the confirmation
        - dialog or false if you don't.    If false, then the window will simply close.   It would
        - be up to you to implement onclick functionality to your 'OK' button.
        -
        - If you are using the ConfirmationDlg then you must have an 'update()' method
        - in your JavaScript which should AJAX call you update/validation SmartView
        -->
