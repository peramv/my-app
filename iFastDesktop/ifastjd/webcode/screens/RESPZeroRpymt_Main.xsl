...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<xsl:with-param name="pageTitle">RESP Account Zero Grant Repayment Information</xsl:with-param>			
		</xsl:call-template>
	</xsl:template>
		
	<!-- ========================== HEADER CALLBACK TEMPLATE =============================-->
	<xsl:template name="header_callback" >
		<link href="{$vURL}css/RESPZeroRpymt.css" rel="stylesheet" type="text/css" />
		<script language="javascript" src="{$vURL}javascript/RESPZeroRpymt.js"></script>		
		<script>
			...
			_modPerm = "<xsl:value-of select="/websrvXML/dtRESPGrantRpymtReloadResponse/userPermissions/modPerm"/>" == "yes";
			_delPerm = "<xsl:value-of select="/websrvXML/dtRESPGrantRpymtReloadResponse/userPermissions/delPerm"/>" == "yes";
			
			_moreRecordsExist = ("<xsl:value-of select="/websrvXML/dtRESPGrantRpymtReloadResponse/RecordRange/moreRecordsExist"/>" == "yes");
			<xsl:if test="/websrvXML/dtRESPGrantRpymtReloadResponse/RecordRange/moreRecordsExist = 'yes'">
				_rangeStartForNext = <xsl:value-of select="/websrvXML/dtRESPGrantRpymtReloadResponse/RecordRange/rangeStartForNext"/>;
			</xsl:if>
		</script>
	</xsl:template>
	...
	<xsl:template name="form_callback" >
		<xsl:variable name="AcctInfoNode" select="/websrvXML/dtRESPGrantRpymtReloadResponse/RESPGrantRpymtInfo/RESPAccountInfo"/>		
				...
				<div class="FieldSetContentsDiv">
					<div id="EntryList_TableDiv" class="Desktop_TableDiv" style="height:120px; width:767px; position:absolute;">
						<table id="EntryList_Table" ifdsTable="true" style="position:absolute; left:0; top:0;" baseClassName="Desktop_Table" cellpadding="0" cellspacing="0" scrollable="true" rowSelectable="true" resizable="true">
							...
							<tbody>
								<xsl:if test="/websrvXML/dtRESPGrantRpymtReloadResponse/RESPGrantRpymtInfo/GrantRpymtRecords">
									<xsl:for-each select="/websrvXML/dtRESPGrantRpymtReloadResponse/RESPGrantRpymtInfo/GrantRpymtRecords/GrantRpymtDetail">
									<tr>
		...
		<div class="ScreenButtonsDiv">					
			<input type="button" value="OK" class="Desktop_Btn" id="btnOK" name="btnOK">				
			</input>											
