...
{	
	...
	var popupCaption = "RESP Grant Repayment - ";
	if ( action == _DELETE )
	{
...
function sendValidateRequest( validateXML, callbackFn )
{
	...
	var paramMap = {}
    paramMap["cz"]                = _vCZ;
    paramMap["jobname"]           = "RESPZeroRpymt_Validate";    
...
function displayResponseError( responseXML )
{
	...
	{
		DesktopError.displayProcessingError( "RESP Account Zero Grant Repayment Information", errorMsg );  
	}
	else
	{
		alert( errorMsg );
...
function buildUpdateXML()
{
	...
	for ( var i = 0 ; i < rowsArray.length; i++ )
	{
		...
		{
			var recordXML = XML.addNode( "GrantRpymtUpd", xml );
...
function sendUpdateRequest( updateXML, callbackFn )
{
	...
	var paramMap = {}
    paramMap["cz"]                = _vCZ;
    paramMap["jobname"]           = "RESPZeroRpymt_Update";    
    paramMap["companyCode"]       = _companyCode;
    paramMap["ruserId"]           = _ruserId;
    paramMap["sessionId"]         = _sessionId;
    paramMap["envName"]           = _envName;
...
function sendReloadRequest( callbackFn )
{
	...
	{		
		...
		try 
		{
			...
			if ( responseXML )
			{
				...
				{					
					dataXML = XML.getNode( "/responseContents/RpymtData", responseXML );
	...
	var paramMap = {}
    paramMap["cz"]                = _vCZ;
    paramMap["jobname"]           = "RESPZeroRpymt_Reload";    
    paramMap["companyCode"]       = _companyCode;
    paramMap["ruserId"]           = _ruserId;
    paramMap["sessionId"]         = _sessionId;
    paramMap["envName"]           = _envName;
...
function updateEntryTable( dataXML )
{
	...
	var dataRecords = XML.getNodes( "GrantRpymtDetail", dataXML );
