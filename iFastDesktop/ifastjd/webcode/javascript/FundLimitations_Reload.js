...
function FundReload(isIt)
{
  ...
  if ( document.getElementById('mainAccountNumber')!= null &&
       document.getElementById('mainAccountNumber').innerHTML != null)
  {
    option="account"; searchtype="ACCOUNT";
  }
  else
  {
    option="brokerBranchRep"; searchtype="BROKER";
  }

  if ( document.getElementById('brokerBox') != null &&
       document.getElementById('brokerBox').code != null)
  {
  ...
  if ( document.getElementById('branchBox') != null &&
       document.getElementById('branchBox').code != null)
  {
  ...
  if ( document.getElementById('repBox') != null &&
       document.getElementById('repBox').code != null)
  {
  ...
  if ( funder == null ||
       funder == 'undefined' ||
       document.getElementById('fundBox').value== null ||
       document.getElementById('fundBox').value== 'undefined' ||
  ...
  if ( isIt == 'more' )
  {
    moreCounter = Number( parseInt(moreCounter) + 100 );
    moreflag=1;
  }
  else
  {
  ...
  if ( document.getElementById('mainAccountNumber') != null &&
       document.getElementById('mainAccountNumber').innerHTML != null )
  {
    _httpRequest.open( "POST",
                       url + "&option=" + option + "&searchtype=" + searchtype + "&AccountNum=" + account_number + "&broker=" + broker +  "&branch=" + branch + "&slsrep=" + salesRep + "&fund=" + funder + "&fundsponsor=" + document.getElementById("fundSponsorDropdown").value + "&class="+classfinal+"&ruletype="+document.getElementById('ruleTypeDropdown').value+"&fundGroup="+document.getElementById('fundGroupBox').value+"&active=" + document.getElementById('onlyActiveBox').checked + "&moreCounter=" + moreCounter,
                       true );

    _httpRequest.onreadystatechange = handleResponse;
    _httpRequest.send();
  }
  else if ( document.getElementById('brokerBox').value != null &&
            document.getElementById('brokerBox').value != "")
  {
    _httpRequest.open("POST",
                      url + "&option=" + option + "&searchtype=" + searchtype + "&AccountNum=" + account_number + "&broker=" + broker +  "&branch=" + branch + "&slsrep=" + salesRep + "&fund=" + funder + "&fundsponsor=" + document.getElementById("fundSponsorDropdown").value + "&class="+classfinal+"&ruletype="+document.getElementById('ruleTypeDropdown').value+"&active=" + document.getElementById('onlyActiveBox').checked + "&moreCounter=" + moreCounter,
                      true );
...
function handleResponse()
{
  if (_httpRequest.readyState == 4)
  {
    if (_httpRequest.status == 200)
    {
      ...
      if ( _httpRequest.responseText.substring( a+19,  b) == null ||
           _httpRequest.responseText.substring( a+19,  b) == "" ||
           _httpRequest.responseText.substring( a+19,  b) == "<websrvXML><reques")
      {
        if (moreflag == 0)
        {
          //clear table
...
//**************************************************************
function addHandleOnKeydown( row, divName )
{
  ...
  var visibileDivHeight = document.getElementById( divName ).clientHeight;
  ...
  {
    if ( (rowPosition) > (scrollTop + visibileDivHeight - 25) &&
         (rowPosition) > (scrollTop + visibileDivHeight - 35) )
    {
      for (var i = 0; i < 15; i++)
      {
        document.getElementById( divName ).scrollTop++;
