...
bool ConfigBrokerObject::selectProfile( const DString &name )
{
	...
	// Select profile from session node in XML configuration file
	selected = selectProfileFromSessionNodeXML(name);

	// If session node is not present in the XML configuration file, select profile from session.grp file
	if( m_pGroupMgr && !selected)
	{
		manager_iterator table_ptr = m_groupTable.find( name );
...
bool ConfigBrokerObject::selectProfileFromSessionNodeXML( const DString &name )
{
	bool selected = false;
	...
	if (NULL != m_pGroupMgr)
	{
		...
		if (NULL != pDSTCMgr)
		{
			...
			DString strIniFileName = I_("");
			DString strIniFileExtension = I_("");

			if (name == I_("Not using AWD"))
			{
				strIniFileName = cfgPath.getValueAsString( I_("NotUse_AWD_File") );
				strIniFileExtension = cfgPath.getValueAsString( I_("NotUse_AWD_Type") );
			}
			else if (name == I_("Use AWD Integration"))
			{
				strIniFileName = cfgPath.getValueAsString( I_("UseIntegration_AWD_File") );
				strIniFileExtension = cfgPath.getValueAsString( I_("UseIntegration_AWD_Type") );
			}
			else if (name == I_("Use AWD Portal Integration"))
			{
				strIniFileName = cfgPath.getValueAsString( I_("UsePortalIntegration_AWD_File") );
				strIniFileExtension = cfgPath.getValueAsString( I_("UsePortalIntegration_AWD_Type") );
			}
			else if (name == I_("Use Call Centre Integration"))
			{
				strIniFileName = cfgPath.getValueAsString( I_("CallCentreIntegration_File") );
				strIniFileExtension = cfgPath.getValueAsString( I_("CallCentreIntegration_Type") );
			}

			if (strIniFileName != I_("") && strIniFileExtension != I_(""))
			{
				selected = true;
				clearTable();

				DString fullFileName = strIniFileName + I_(".") + strIniFileExtension;

				ConfigValue cvPath( path );
				ConfigValue cvFile( fullFileName );
...
bool ConfigBrokerObject::removeFromRegistry( const DString &configuration )
{
   TRACE_METHOD( I_( "ConfigBrokerObject" ), I_( "destroySubKey()" ) )
...
/*
   ...
   {
		DString registryPath = buildApplicationPath( m_appName );
		   
		RegistryKey local( registryPath.asA(), m_hive );
		destroyed = local.deleteSubKey( configuration );
   }
*/
   return(destroyed);
}
...
//******************************************************************************
//
// $Log:   Y:/VCS/BF Core/ifdscore/configbrokerobject.cpp-arc  $
//
//   Rev 1.2   Sep 22 2005 16:45:44   fengyong
