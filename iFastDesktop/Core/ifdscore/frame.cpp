...
void Frame::addConditionFromObject(CConditionObject& object, 
                                  const char *fileName,
                                  unsigned lineNumber, 
                                  BOOL baseCondition,
                                  SEVERITY severity,
...
{
	DString dstr = object.GetConditionName();
	I_CHAR* codeStr = &dstr.getImp()[0];

	long code = getConditionIDForObject(dstr);
   ...
   condition->readFromObject(object);
...
void Frame::packConditions( void )
{
   ...
   typedef std::set< int, std::greater<int> > SETINT;
...
long Frame::generateConditionID() const
{
	std::random_device seed_gen;
	std::default_random_engine dre(seed_gen());
	std::uniform_int_distribution<int> di(3000000, 3100000);

	long conditionID = (long)di(dre);
	
	return conditionID;
}

long Frame::getConditionIDForObject(const DString& conditionName)
{
	long conditionID = -1;
	auto searchValue = mapConditionNameToID.find(conditionName);

	if (searchValue != mapConditionNameToID.end())
	{
		conditionID = searchValue->second;
	}
	else
	{
		conditionID = generateConditionID();
		mapConditionNameToID.insert(std::pair<DString, long>(conditionName, conditionID));
	}
...
//**********************************************************************
//
// $Log:   Y:/VCS/BF iFAST/core/ifdscore/frame.cpp-arc  $
//
