...
, _clientLocale( 0 )
, _init( false )
, _initFromObject( copy._initFromObject )
{
...
MsgPair * getMsgPairFromCndCode( MsgPair * pair, long code )
{
   int i = 0;
   for( i = 0;
      Condition::NO_CONDITION_CODE != pair[ i ].msgCode;
      ++i )
   {
...
bool Condition::readFromObject(CConditionObject& conditionObj)
{
    bool fProblem = true;

    DString dstrLocale = _clientLocale->getLocale();
    DString dstrClient = _clientLocale->getClient();
    DString dstrMarket = _clientLocale->getMarket();
    long defaultMarket = _clientLocale->getMarketIndex();
    long defaultClient = _clientLocale->getClientIndex();

    conditionObj.SetClientName(dstrClient);
    conditionObj.SetMarketName(dstrMarket);

    CConditionBuilder* pConditionBuilder = CConditionBuilder::Instance();

    if (pConditionBuilder != NULL)
    {
        pConditionBuilder->CheckForOverrides(&conditionObj, _pathName);
	}

    _severity = (SEVERITY)conditionObj.GetSeverityOverride();
    if (_severity == -1)
    {
        _severity = (SEVERITY)conditionObj.GetSeverity();
    }

    // There is no override for suppress
    _suppress = conditionObj.GetSuppress();

    if (dstrLocale == I_( "enUS" ))
    {
        _message = conditionObj.GetMessageOverride_EN_US();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_EN_US();
        }

        _correction = conditionObj.GetActionOverride_EN_US();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_EN_US();
        }
    }
    else if (dstrLocale == I_( "frFR" ))
    {
        _message = conditionObj.GetMessageOverride_FR_FR();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_FR_FR();
        }

        _correction = conditionObj.GetActionOverride_FR_FR();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_FR_FR();
        }
    }
    else if (dstrLocale == I_( "deDE" ))
    {
        _message = conditionObj.GetMessageOverride_DE_DE();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_DE_DE();
        }

        _correction = conditionObj.GetActionOverride_DE_DE();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_DE_DE();
        }
    }
    else if (dstrLocale == I_( "esES" ))
    {
        _message = conditionObj.GetMessageOverride_ES_ES();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_ES_ES();
        }

        _correction = conditionObj.GetActionOverride_ES_ES();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_ES_ES();
        }
    }
    else if (dstrLocale == I_( "frCA" ))
    {
        _message = conditionObj.GetMessageOverride_FR_CA();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_FR_CA();
        }

        _correction = conditionObj.GetActionOverride_FR_CA();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_FR_CA();
        }
    }
    else if (dstrLocale == I_( "nlNL" ))
    {
        _message = conditionObj.GetMessageOverride_NL_NL();
        if (_message == I_(""))
        {
            _message = conditionObj.GetMessage_NL_NL();
        }

        _correction = conditionObj.GetActionOverride_NL_NL();
        if (_correction == I_(""))
        {
            _correction = conditionObj.GetAction_NL_NL();
        }
    }
...
bool Condition::readFromFile( BOOL baseFlag, bool bUseDefaultLang )
{
    ...
    if (NULL != pConditionBuilder)
    {
        ...
        CConditionObject* pCondition = pConditionBuilder->GetCondition(_code, _componentName, dstrLocale, dstrClient, dstrMarket, _pathName);

        if (pCondition != NULL)
        {
            _severity = (SEVERITY)pCondition->GetSeverityOverride();
            if (_severity == -1)
            {
                _severity = (SEVERITY)pCondition->GetSeverity();
            }

            // There is no override for suppress
            _suppress = pCondition->GetSuppress();

            if (dstrLocale == I_( "enUS" ))
            {
                _message = pCondition->GetMessageOverride_EN_US();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_EN_US();
                }

                _correction = pCondition->GetActionOverride_EN_US();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_EN_US();
                }
            }
            else if (dstrLocale == I_( "frFR" ))
            {
                _message = pCondition->GetMessageOverride_FR_FR();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_FR_FR();
                }

                _correction = pCondition->GetActionOverride_FR_FR();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_FR_FR();
                }
            }
            else if (dstrLocale == I_( "deDE" ))
            {
                _message = pCondition->GetMessageOverride_DE_DE();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_DE_DE();
                }

                _correction = pCondition->GetActionOverride_DE_DE();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_DE_DE();
                }
            }
            else if (dstrLocale == I_( "esES" ))
            {
                _message = pCondition->GetMessageOverride_ES_ES();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_ES_ES();
                }

                _correction = pCondition->GetActionOverride_ES_ES();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_ES_ES();
                }
            }
            else if (dstrLocale == I_( "frCA" ))
            {
                _message = pCondition->GetMessageOverride_FR_CA();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_FR_CA();
                }

                _correction = pCondition->GetActionOverride_FR_CA();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_FR_CA();
                }
            }
            else if (dstrLocale == I_( "nlNL" ))
            {
                _message = pCondition->GetMessageOverride_NL_NL();
                if (_message == I_(""))
                {
                    _message = pCondition->GetMessage_NL_NL();
                }

                _correction = pCondition->GetActionOverride_NL_NL();
                if (_correction == I_(""))
                {
                    _correction = pCondition->GetAction_NL_NL();
                }
            }
        ...
        else
        {
            ...
            _message.append(I_( "Condition could not be retrieved.  " ));
...
DString Condition::getDiagnosticMessage() const
{
   ...
   wss << DString( DStringA(getFileName()) ) << I_(" [") << getLineNumber() << I_("]") << std::endl;
   return(wss.str());
}
...
//************************************************************************
//
// $Log:   Y:/VCS/BF iFAST/core/ifdscore/condition.cpp-arc  $
//
