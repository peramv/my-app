...
public:
   ...
   bool RemoveConfig( const DString &configuration );

   static void setClientName( const DString &name ) { m_clientname = name; };
   static const DString getClientName () { return m_clientname; };

   static void SetSaveDesktopWebURL(BOOL bSave);
   static BOOL GetSaveDesktopWebURL();
...
private:
   ...
   manager_map         m_table;
   DString             m_appName;
   static DString	   m_clientname;
   MutexSemaphore     *m_mutex;
   static BOOL		   m_bSaveDesktopWebURL;
 ...
 * steps for getting the value.  The first variant (without the configManager
 * parameter) assumes that the configManager is the application name.
 *
 * The key should be a string.  If the value of the string is "1", "YES", "TRUE", 
 * or "ON" the value is treated as TRUE.
 *
 ...
 */
DICORE_LINKAGE bool GetConfigValueAsBool( const DString & configuration,
                                          const DString & key );
DICORE_LINKAGE bool GetConfigValueAsBool( const DString & configManager,
                                          const DString & configuration,
                                          const DString & key );
/**
 * A common interface to get configuration data.  Performs the three
 * steps for getting the value.  Assumes that the configManager is the 
 * application name
 *
 * @param configuration - Name of the configuration to retrieve
 * @param key           - Name of the key whose value you wish
 ...
 */
DICORE_LINKAGE DString GetConfigValueAsString( const DString & configuration,
                                               const DString & key );
 ...
 */
DICORE_LINKAGE int GetConfigValueAsInteger( const DString & configuration,
                                            const DString & key );
 ...
 */
DICORE_LINKAGE long GetConfigValueAsLong( const DString & configuration,
                                          const DString & key );
 ...
 */
DICORE_LINKAGE double GetConfigValueAsDouble( const DString & configuration,
                                              const DString & key );
...
//******************************************************************************
//
// $Log:   Y:/VCS/BF iFAST/core/ifdscore/configmanager.hpp-arc  $
//
//   Rev 1.4   Feb 27 2009 08:49:38   purdyech
//ZTS Fixes prior to User Experience Test
//
//   Rev 1.3   Jul 07 2006 12:54:18   FENGYONG
//Incident #659311 - add client name
//
//   Rev 1.2   Sep 22 2005 16:46:36   fengyong
//Add feature - remove config within an config manager
//
//   Rev 1.1   Oct 09 2002 17:41:50   PURDYECH
//New PVCS Database
//
//   Rev 1.0   Jul 05 2002 10:14:40   SMITHDAV
//Initial revision.
// 
