...
DString ConfigManager::m_clientname = NULL_STRING;
BOOL ConfigManager::m_bSaveDesktopWebURL = FALSE;
...
bool ConfigManager::RemoveConfig( const DString &configuration )
{
   bool destroyed = m_pBroker->removeFromRegistry( configuration );
   return(destroyed);
}
...
void ConfigManager::SetSaveDesktopWebURL(BOOL bSave)
{
	m_bSaveDesktopWebURL = bSave;
}

BOOL ConfigManager::GetSaveDesktopWebURL()
{
	return m_bSaveDesktopWebURL;
}
...
bool GetConfigValueAsBool( const DString &configuration,
                           const DString &key )
{
   return( GetConfigValueAsBool( I_("MFDSTC") /*ConfigManager::getApplication()*/,
                                 configuration,
                                 key ) );
}

bool GetConfigValueAsBool( const DString &configManager,
                           const DString &configuration,
                           const DString &key )
{
   DString dstrResult = GetConfigValueAsString( configManager, configuration, key );
   dstrResult.lowerCase();
   if( dstrResult == I_("1") ||
       dstrResult == I_("true") ||
       dstrResult == I_("on") ||
       dstrResult == I_("yes") ) {
...
DString GetConfigValueAsString( const DString &configuration,
                                const DString &key )
{
   return( GetConfigValueAsString( I_("MFDSTC") /* ConfigManager::getApplication()*/,
                                   configuration,
                                   key ) );
}
...
int GetConfigValueAsInteger( const DString &configuration,
                                const DString &key )
{
   return( GetConfigValueAsInteger( I_("MFDSTC") /* ConfigManager::getApplication() */,
                                    configuration,
                                    key ) );
}
...
long GetConfigValueAsLong( const DString &configuration,
                           const DString &key )
{
   return( GetConfigValueAsLong( I_("MFDSTC") /* ConfigManager::getApplication() */,
                                 configuration,
                                 key ) );
}
...
double GetConfigValueAsDouble( const DString &configuration,
                               const DString &key )
{
   return( GetConfigValueAsDouble( I_("MFDSTC") /* ConfigManager::getApplication() */,
                                   configuration,
                                   key ) );
}
...
//******************************************************************************
//
// $Log:   Y:/VCS/BF iFAST/core/ifdscore/configmanager.cpp-arc  $
//
//   Rev 1.5   Nov 03 2009 03:48:00   purdyech
...
//
//   Rev 1.4   Feb 27 2009 08:49:30   purdyech
//ZTS Fixes prior to User Experience Test
//
//   Rev 1.3   Jul 07 2006 12:53:50   FENGYONG
//Incident #659311 - add client name
//
//   Rev 1.2   Sep 22 2005 16:46:30   fengyong
//Add feature - remove config within an config manager
// 
//    Rev 1.1   Oct 09 2002 17:41:50   PURDYECH
// New PVCS Database
// 
//    Rev 1.0   Jul 05 2002 10:12:48   SMITHDAV
// Initial revision.
// 
