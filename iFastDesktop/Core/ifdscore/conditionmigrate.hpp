...
#ifdef INFRA_CLIENTLOCALE
   #define DEFAULT_CLIENTLOCALE NULL
#else
   #include <bfutil\clientlocalecontext.hpp>
   #define DEFAULT_CLIENTLOCALE &ClientLocaleContext::get()
#endif

#if !defined(NO_EXECHIST_TRACING)
...
#define ADDCONDITIONFROMOBJECT( object ) \
   cmFrame.getFrame()->addConditionFromObject( object, \
                                             __FILE__, \
                                             __LINE__, \
                                             FALSE, \
                                             NO_SEVERITY, NULL_STRING, DEFAULT_CLIENTLOCALE );

#define ADDCONDITIONFROMOBJECTIDI( object, str1 ) \
   cmFrame.getFrame()->addConditionFromObject( object, \
                                             __FILE__, \
                                             __LINE__, \
                                             FALSE, \
...
//**********************************************************************
//
// $Log:   Y:/VCS/BF iFAST/core/ifdscore/conditionmigrate.hpp-arc  $
//
