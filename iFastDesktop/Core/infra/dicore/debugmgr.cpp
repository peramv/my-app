                                           ...
                                           DString& strMsgHeader )
{
   ...
   JavaDate *_curTimeDate = new JavaDate();
   DIINT64 milliTime = _curTimeDate -> getTime();

   JavaLong *milliTimeLong = new JavaLong( milliTime );
   DString milliSecondStr = milliTimeLong->toString();

   milliSecondStr.padLeft( MILLISECOND_LEN, '0' );

   delete milliTimeLong;
   delete _curTimeDate;
...
bool DebugMgr::connectTrace()
{
   ...
   if ( ! ( _pIPC || _logFile ) )
   {
      ...
      if ( SOCKET_VALUE == traceMethodString )
      {
         ...
         try
         {
            _pIPC = new Socket( DStringA(IPAddressString).c_str(), (unsigned short)portNumber );
         }
         catch(...)
         {
            termTrace();
...
void DebugMgr::initTrace()
{
   ...
   if ( DString( ON_VALUE ) == initialValue )
   {
      _fIsTracing = connectTrace();
   }
...
//
//
// $Log:   Y:/VCS/iFastAWD/dicore/debugmgr.cpp-arc  $
// 
//    Rev 1.0   28 Jan 2008 13:19:34   kovacsro
// Initial revision.
// 
//    Rev 1.13   30 Aug 2001 15:45:58   dt14177
// Boundschecker warning fix: created a
// DString from ON_VALUE
// 
