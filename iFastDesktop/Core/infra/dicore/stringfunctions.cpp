...
bool isDecimal( const I_STRING & aString )
{
   ...
   if ( ! isDecimal )
   {
      ...
      if ( firstChar == NEGATIVE_SIGN )
      {
         ...
         isDecimal = (unsigned int)( hasDecimal ? 2 : 1 ) < strLength;
      }
   ...
   if ( hasDecimal )
   {
      ...
      for ( unsigned int index = decPtPos + 1; index < strLength; ++index )
      {
...
{
   unsigned int nStringLen = aString.length();
...
DString &padLeft( DString &aString, unsigned long padLength, I_CHAR padChar )
{
...
DString &strip( DString &aString, I_CHAR stripChar )
{
   return stripLeading( stripTrailing( aString, stripChar ), stripChar );
}
...
DString &stripAll( DString &aString, I_CHAR stripChar )
{
...
DString &stripLeading( DString &aString, I_CHAR stripChar )
{
...
DString &stripTrailing( DString &aString, I_CHAR stripChar )
{
...
DString& upperCase( DString& aString )
{
   upperCase(aString.getImp());
   return aString;
}
...
I_STRING floatAsString( double nValue )
{
   I_CHAR szValue[ 43 ];
   I_STRING saveString;
   i_sprintf( szValue, I_( "%20.20f" ), nValue );
   saveString = szValue;
   stripTrailing( saveString, '0' );
   return saveString;
}
...
/**
     A function to assist in formatting a string with a Mask without relying upon Locale. 
	 Preferred method for format.
     @param valueString - string to be altered
	 @param mask - string to use for the mask

*/
I_STRING & formatStringUsingMaskICU( I_STRING &valueString, const I_STRING &mask )
{
   ...
   //USING ICU for currency and Decimal
    UErrorCode success = U_ZERO_ERROR;
    UNumberFormatSymbol currForm;
    currForm = UNUM_CURRENCY_SYMBOL;
    UNumberFormatSymbol decimalForm = UNUM_DECIMAL_SEPARATOR_SYMBOL;
    UNumberFormat * fmt= unum_open(UNUM_DEFAULT, NULL, -1, NULL, NULL, &success);
    UChar buffer [1000];
	UChar bufferDec [1000];
    int32_t size= 1000;  
	unum_getSymbol( fmt, decimalForm, bufferDec, size, &success ); //For ICU decimal symbol
	UnicodeString bufferDecStr(bufferDec);
	I_STRING bufferDecD;
	ICUStringtoIString( bufferDecStr, bufferDecD );
	const I_CHAR * bufferI = bufferDecD.c_str();
   // I_CHAR newBuff[1000]; 
   //u_austrcpy(newBuff, bufferDec);
   ...
   if ( maskLen > 0 )
   {
          ...
          I_STRING::npos != mask.find( I_( "." ) ))
      {
         ...
         if ( I_STRING::npos == mask.find_first_not_of( I_( "%." ) ) )
         {
            ...
            if ( I_STRING::npos == valueString.find( bufferDecStr[0]) )
            {
               valueString += bufferDecStr[0]; 
            }
            ...
            if ( valueString.length() < strippedMask.length() )
            {
               ...
               long valueBeforeDec = valueString.find( bufferI );
            ...
            long decPlace = valueString.find( bufferDecStr[ 0 ]);
            long afterDecimal = newLen - ( decPlace - 1 );
            if ( afterDecimal < 2 )
            {
               padRight( valueString, ( 2 - afterDecimal ), '0' );
            }
            valueString.insert( decPlace + 3, bufferI) ;
         ...
         long oldDecPlace = valueString.find( bufferDecStr[ 0 ]);
         if ( I_STRING::npos != oldDecPlace )
         {
            stripLeading( valueString, '0' );
            if ( valueString.find( bufferDecStr[ 0 ])== 0 )
            {
         ...
         if ( I_STRING::npos != ( srcOffset = valueString.find( bufferDecStr[ 0 ] )))
         {
         ...
         while ( maskOffset >= 0 )
         {
            ...
            if ( I_STRING::npos != maskVals.find( thisMaskChar ))
            {
               ...
               else
               {
                  ...
                  {
                     case '$' :
						unum_getSymbol( fmt,currForm, buffer, size, &success );
						newValue[ targetOffset-- ] = buffer[ 0 ];
                        maskOffset = 0;
                        break;

                     case '.' :
                        newValue[ targetOffset-- ] = bufferDecStr[ 0 ];
                        break;
                  ...
                  else if ( ',' == thisMaskChar )
                  {
                     switch ( mask[ maskOffset + 1 ] )
                     {
                        ...
                        case 'Z':
                           ...
                           {
                              case '$':
                                 replaceChar = buffer[0];
                                    break;
                              case '.':
                                 replaceChar = bufferDecStr[ 0 ];
                                    break;
                              default:
...
/**
     A function to assist in removing a Mask without relying upon Locale. Preferred method for
	 removing a mask.
     @param maskedvalue - string to be altered
	 @param mask - string to use to remove the mask
*/

I_STRING &removeMaskFromStringICU( I_STRING &maskedValue, const I_STRING &mask )
{
   ...
   //USING ICU for currency and Decimal
    ...
    int32_t size= 1000; 
	unum_getSymbol( fmt, decimalForm, bufferDec, size, &success ); //For ICU decimal symbol
	unum_getSymbol( fmt, currForm, buffer, size, &success ); //For ICU currency symbol
	UnicodeString bufferDecStr(bufferDec);
	I_STRING bufferDecD;
	ICUStringtoIString( bufferDecStr, bufferDecD );
	const I_CHAR * bufferI = bufferDecD.c_str();
...
//	u_austrcpy(newBuff, bufferDec);
          ...
          I_STRING::npos != mask.find( I_( "%" ) ) ) && ( I_STRING::npos != mask.find( I_( "." ))))
   {
      ...
      if ( I_STRING::npos != thisMask.find_first_not_of( I_( "%." )))
      {
         ...
         if ( I_STRING::npos == maskedValue.find( bufferDec[0] ))
         {           
			stripTrailing( maskedValue );
            maskedValue = maskedValue +  bufferI; 
            valLength = maskedValue.length();
         }
         long srcDecPos = maskedValue.find( bufferDec[0] ); 
...
/**
     A function to assist in formatting a Mask using Locale. 
     @param valueString - string to be altered
	 @param mask - string to use for the mask
	 @see Locale
	 @deprecated - please refer to formatStringUsingMaskICU
*/
...
/**
     A function to assist in removing a Mask using Locale. 
     @param maskedValue - string to be altered
	 @param mask - string to use for the mask
	 @see Locale
	 @deprecated -please refer to removeMaskFromStringICU()
*/
...
{
   const int strLength = aString.length();
   ...
   // Find a decimal point
   unsigned int decPtPos = aString.find( (char)DECIMAL_POINT );
   bool hasDecimal = I_STRING::npos != decPtPos;
   ...
   if ( ( CHAR_MAX != GROUPING ) && ( '\0' != DIGIT_SEPARATOR ) )
   {
      hasSeparator = I_STRING::npos != aString.find( (char)DIGIT_SEPARATOR );
   }
...
/**
     A function to assist in formatting a Mask without relying upon Locale. 
	 Preferred method for formatting a string.
     @param valueString - string to be altered
	 @param mask - string to use for the mask

*/
DStringA & formatStringUsingMaskICU( DStringA &valueString, const DStringA &mask )
{
    DStringA valueStringU(valueString);
    DStringA maskU(mask);
	
	DStringA newValue( maskU.length(), ' ' ),
            maskVals( "A9#XZ$%" );
   long  valDecimalPosition = 0, maskDecimalPosition = 0, srcOffset = 0,
         targetOffset = 0, maskOffset = 0, maskValsLen = 0, valLen = 0,
         maskLen = 0;
   ...
   //Using ICU for currency
    UErrorCode success = U_ZERO_ERROR;
    UNumberFormatSymbol currForm;
    currForm = UNUM_CURRENCY_SYMBOL;
    UNumberFormatSymbol decimalForm = UNUM_DECIMAL_SEPARATOR_SYMBOL;
    UNumberFormat * fmt= unum_open(UNUM_DEFAULT, NULL, -1, NULL, NULL, &success);
	I_CHAR bufferDec [1000];
    UChar buffer [1000];
    int32_t size= 1000; 
	unum_getSymbol( fmt, decimalForm, bufferDec, size, &success ); //For ICU decimal symbol
	DStringA bufferDecA( bufferDec );

   maskLen = maskU.length();
   maskValsLen = maskVals.length();

   if ( maskLen > 0 )
   {
      if ( ( ( DStringA::npos != maskU.find( "$" )) ||
          ( DStringA::npos != maskU.find( "9" )) ||
          ( DStringA::npos != maskU.find( "%"))) &&
          DStringA::npos != maskU.find( "." ))
      {
         strip( valueStringU );
         if ( DStringA::npos == maskU.find_first_not_of( "%." ) )
         {
            
			 stripAll( valueStringU, '%' );

            if ( DStringA::npos == valueStringU.find( (char)bufferDec[0]) )
            {
               valueStringU += (char)bufferDec[0];
            }
            DStringA strippedMask( maskU );
            strip( strippedMask );
            if ( valueStringU.length() < strippedMask.length() )
            {
			   long beforeDec = strippedMask.find( "." );
               long afterDec = strippedMask.length() - ( beforeDec + 1 );
               long valueBeforeDec = valueStringU.find( bufferDecA ); //char*
               long valueAfterDec = valueStringU.length() - ( valueBeforeDec + 1 );
               if ( valueBeforeDec < beforeDec )
               {
                  padLeft( valueStringU, ( valueStringU.length() + ( beforeDec - valueBeforeDec )), '0' );
               }
               if ( valueAfterDec < afterDec )
               {
                  padRight( valueStringU, ( valueStringU.length() + ( afterDec - valueAfterDec )), '0' );
               }
            }
            long newLen = valueStringU.length();
            long decPlace = valueStringU.find( bufferDecA );
            long afterDecimal = newLen - ( decPlace - 1 );
            if ( afterDecimal < 2 )
            {
               padRight( valueStringU, ( 2 - afterDecimal ), '0' );
            }
            valueStringU.insert( decPlace + 3, bufferDecA );
            valueStringU.erase( decPlace, 1 );
         }
         if ( DStringA::npos != valueStringU.find( "-"))
         {
            isNegative = true;
            valueStringU = valueStringU.substr( 1 );
         }

         long oldDecPlace = valueStringU.find( bufferDecA );
         if ( DStringA::npos != oldDecPlace )
         {
            stripLeading( valueString, '0' );
            if ( valueStringU.find( bufferDecA ) == 0 )//kg remove ';'
            {
               valueStringU = "0" + valueStringU;
            }
         }

         valLen = valueStringU.length();
         if ( DStringA::npos == ( maskOffset = maskU.find( ".")))
         {
            maskOffset = maskLen - 1;
         }
         ...
         if ( DStringA::npos != ( srcOffset = valueStringU.find( bufferDecA )))
         {
            valDecimalPosition = srcOffset--;
         }
         else
         ...
         while ( maskOffset >= 0 )
         {
            thisMaskChar = maskU[ maskOffset ];
            if ( DStringA::npos != maskVals.find( thisMaskChar ))
            {
               if ( srcOffset >= 0 )
               {
                  newValue[ targetOffset-- ] = valueStringU[ srcOffset-- ];
               }
               else
               {
                  switch ( thisMaskChar )
                  {
                     case '$' :
						unum_getSymbol( fmt,currForm, buffer, size, &success );
                        newValue[ targetOffset-- ] = (char)buffer[ 0 ];
                        maskOffset = 0;
                        break;

                     case '.' :
                        newValue[ targetOffset-- ] = bufferDecA[ 0 ];
                        break;
            ...
            {
               unum_getSymbol( fmt,currForm, buffer, size, &success );//for ICU currency
			   if ( srcOffset >= 0 )
               {
                  if ( '-' != valueStringU[ srcOffset ] )
                  {
                     newValue[ targetOffset-- ] = thisMaskChar;
                  }
                  else if ( '.' == thisMaskChar )
                  ...
                  else if ( ',' == thisMaskChar )
                  {
                     switch ( maskU[ maskOffset + 1 ] )
                     {
                        ...
                        case 'Z':
                           ...
                           {
                              case '$':
                                 replaceChar = (char)buffer[0];
                                    break;
                              case '.':
                                 replaceChar = bufferDecA[ 0 ];
                                    break;
                              default:
                                 replaceChar =  thisMaskChar;
                                 break;
      ...
      else
      {
         ...
         while ( maskOffset < maskLen )
         {
            if ( DStringA::npos != maskVals.find( maskU[ maskOffset ] ))
            {
               if ( srcOffset < valLen )
               {
                  newValue[ targetOffset++ ] = valueStringU[ srcOffset++ ];
            ...
            {
               newValue[ targetOffset++ ] = maskU[ maskOffset ];
            }
            ++maskOffset;
         }
         valueStringU = newValue;
   ...
   valueString = valueStringU;
   return valueString;
}
/**
     A function to assist in removing a Mask without relying upon Locale. 
	 Preferred method for removing a mask.
     @param maskedValue - string to be altered
	 @param mask - string to use for the mask

*/

DStringA &removeMaskFromStringICU( DStringA &maskedValue, const DStringA &mask )
{
   long valLength = maskedValue.length();
   long maskLength = mask.length();
...
//Using ICU for currency and Decimal
          ...
          DStringA::npos != mask.find( "%" ) ) && ( DStringA::npos != mask.find( ".")))
   {
      ...
      if ( DStringA::npos != thisMask.find_first_not_of( "%."))
      {
         ...
         if ( DStringA::npos == maskedValue.find( bufferDecA ))
         {
            stripTrailing( maskedValue );
            maskedValue = maskedValue + bufferDecA;
            valLength = maskedValue.length();
         }
         long srcDecPos = maskedValue.find( bufferDecA ); 
         ...
         if ( hasDec )
         {
            while ( srcOffset >= 0 && maskOffset >= 0 )
            {
               ...
               else
               {
                  if ( maskedAtOffset != thisMask[ maskOffset ] )
                  {
					  if ( maskedAtOffset != bufferDecA[ 0 ] || 
                          maskOffset == 0 ||
                          thisMask[ maskOffset - 1 ] != '$' )
                     {
                        targetValue[ targetOffset-- ] = maskedAtOffset;
...
/**
     A function using Locale to assist in formating a Mask
     @param valueString - string to be altered
	 @param mask - string to use for the mask
	 @see Locale
	 @deprecated -please refer to formatStringUsingMaskICU()
...
/**
     A function using Locale to assist in removing a Mask
     @param maskedvalue - string to be altered
	 @param mask - string to use for the mask
	 @see Locale
	 @deprecated -please refer to removeMaskFromStringICU()
...
//
//
// $Log:   Y:/VCS/iFastAWD/dicore/stringfunctions.cpp-arc  $
// 
//    Rev 1.0   28 Jan 2008 13:20:30   kovacsro
// Initial revision.
// 
//    Rev 1.27   Apr 08 2002 12:47:52   dt40784
// more Uni and ASCii compatibility
// 
//    Rev 1.26   Apr 05 2002 18:05:20   dt40784
// added unicode functionality
// 
//    Rev 1.23   Apr 03 2002 16:53:46   dt40784
// added methods formatstringusingMaskICU and removeMaskfromStringICU
// to remove necessity of Locale
// 
//    Rev 1.22   Jun 19 2001 11:36:40   DT11159
// Fixed unicode problem in floatAsString.
//
//    Rev 1.21   13 Jun 2001 16:37:50   dt14177
// fixed problem with stringfromfloat so
// it uses sprintf instead of _gcvt
//
