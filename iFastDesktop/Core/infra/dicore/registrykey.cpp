...
typedef void (__cdecl *PFNGETOVERRIDES)( PFRCO*, PFRGOC*, PFREOV*, PFREOK* );

void RegistryKey::LoadOverrideFunctions() {
   I_CHAR szModName[256] = I_("ifdscore.dll");
#ifdef DEBUG
   i_strcpy( szModName, I_("ifdscore_d.dll") );
#endif
   HMODULE hModule;
   PFNGETOVERRIDES pfn = NULL;
   hModule = GetModuleHandleW((LPCWSTR)szModName);
   if( NULL != hModule ) {
      pfn = (PFNGETOVERRIDES)GetProcAddress(hModule, "GetOverrideFunctions");
      if( NULL != pfn ) {

         pfn(&s_pfnCheckOverride,
            &s_pfnGetOverrideCount,
            &s_pfnEnumOverrideValue,
            &s_pfnEnumOverrideKey);
      }
      else {
         ...
         case LOCAL_MACHINE:
            {
               ...
               strcpy_s( szName, _machineName.c_str() );
...
//******************************************************************************
//
// $Log:   Y:/VCS/iFastAWD/dicore/registrykey.cpp-arc  $
// 
//    Rev 1.1   Nov 02 2011 04:43:06   purdyech
// Registry Cleanup
//
