...
#  pragma message( "Text Type: UNICODE" )
#  define _UNICODE
...
#  pragma message( "Text Type: MULTIBYTE" )
...
#  pragma message( "Text Type: ASCII" )
...
inline void ICUStringtoAString( const UnicodeString &uStr,
                                        A_STRING & aStr )
{
   unsigned length = uStr.length();
   char *save = new char[ length + 1 ];

   uStr.extract(0, length, save );
...
void ICUStringtoIString( const UnicodeString & uStr,
                         I_STRING & iStr );

inline void ICUStringtoIString( const UnicodeString & uStr,
                                I_STRING & iStr )
{
   unsigned length = uStr.length();
   I_CHAR *save = new I_CHAR[ length + 1 ];
...
//
//
// $Log:   Y:/VCS/iFastAWD/ibmcppw/include/commonport.h-arc  $
//
//   Rev 1.0   28 Jan 2008 13:25:42   kovacsro
//Initial revision.
// 
