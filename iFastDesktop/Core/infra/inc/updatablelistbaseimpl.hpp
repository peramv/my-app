#ifndef UPDATABLELISTBASEIMPL_HPP
#define UPDATABLELISTBASEIMPL_HPP
...
/** "Hidden" class used to hold activity and pointer to CBOBase 
 *  object
 */
class CBOBASE_LINKAGE updListItem
{
   public:

    updListItem();
    updListItem( const updListItem& copy ); 
    ...
    Data *        pData;
    ACTIVITY      activity;
    bool          bDeleteInstance;
    unsigned long ulFlags;
};

// Standard typedefs
typedef map< DString, updListItem*, less<DString> >::const_iterator UPDLIST_ITER;
typedef map< DString, updListItem*, less<DString> >  UPDLIST_MAP;
typedef UPDLIST_MAP::value_type UPDLIST_VALUE_TYPE;

// Rollback Map
typedef map< DString, UPDLIST_MAP *, less<DString> >::const_iterator LABELED_RB_ITER;
typedef map< DString, UPDLIST_MAP *, less<DString> >  LABELED_RB_MAP;


/**
 * UpdatableListBaseImpl is a base class used for
 ...
 * ITEM_DELETED
 * 
 * UpdatableListBaseImpl offers an interface in the form of
 ...
 * to perform updates.
 * 
 * UpdatableListBaseImpl rollback capability :
 * The UpdatableListBaseImpl allows you to roll the list & activity back to a set checkpoint
 * Using the methods setCheckpointForRollBack(const string&)  & rollBackListToCheckpoint(const string&)
 * 
 ...
 * @see CBEBase
 */
class CBOBASE_LINKAGE UpdatableListBaseImpl : public ListBaseImpl
{
   public:
   ...
   /**
     ...
     * @param iter       A UPDLIST_ITER iterator that should point to a
     *                   position in the collection of CBO/Data objects.
     * @param strKey     A unique string tag indentifier for the registered field
     ...
     * @see Data
     */
    virtual SEVERITY getField( UPDLIST_ITER iter, 
                               const DString & strField, 
                               DString & strOut,
                               bool fFormatted = false) const;
     ...
     * @param iter      a UPDLIST_ITER iterator that should point to a
     *                  position in the collection of CBO/Data objects.
     *                  This parm allow you to specify a starting position.
     * @return a UPDLIST_ITER iterator that should point to a
     *         position in the collection of CBO/Data objects.
     *         return iter.end() if not found
     */
    UPDLIST_ITER getMatchingPositionFrom( 
     ...
     * @see setObject
     */
    virtual bool getNextKey(DString &strKey);
     ...
     * the collection of CBO/Data objects.
     * 
     * @param x      a UPDLIST_ITER iterator that should point to a
     *               position in the collection of CBO/Data objects.
     * @return A pointer to a CBOBase object.
     * @see setObject
     * @see deleteObject
     */
    virtual CBOBase * getObject(UPDLIST_ITER x);
     ...
     * @see deleteObject
     */
    virtual SEVERITY getObject( UPDLIST_ITER x,
                                CBOBase & rObject );
    ...
    /**
     * Determine if the item at the speciefied position
     ...
     * @see setObject
     */
    bool     isUpdated(UPDLIST_ITER x);

    /**
     * Method used to make a FULL Copy of an UpdatableListBaseImpl pointer 
     * 
     * @param copy
     * @return Deep Copy of an UpdatableListBaseImpl pointer
     */
    virtual UpdatableListBaseImpl * makeCopy( 
                             const UpdatableListBaseImpl & copy,
                             UpdatableListBaseImpl * pDeepCopy );
     ...
     * @return NO_CONDITION if successful
     */
    virtual SEVERITY modelFrom( 
                           const UpdatableListBaseImpl * pCopy );
     ...
     * @see getData
     */
    virtual SEVERITY performIsValidForUpdateEdits( 
                                             UPDLIST_ITER & iter,
     ...
     */
    virtual SEVERITY populateListWithDataObjects( );
     ...
     * @see ACTIVITY
     */
    virtual SEVERITY removeObjectAndActivity(UPDLIST_ITER x);
     ...
     * @see getContainer
     */
    virtual void setContainer( CBOBase* pContainer );
    ...
    /** virtual destructor */
    virtual ~UpdatableListBaseImpl();
    

   protected: //method

    /**
     * Sets a miscellaneous bit flag value for an item in the
     * CBO/Data object list.
     * 
     * @param strKey    A unique string key value used for searching
     *                  the collection of CBO/Data objects. This key
     *                  should be in the format of an IDI string, example:
     *                  Fund_Code=0000001;Account_Number=00000054961;
     * @param ulAddFlag a long value to be set as a bit flag
     * @return true if successful
     *         false if failure
     * @see getObjectFlag
     * @see isObjectFlagSet
     * @see removeObjectFlag
     */
    bool addObjectFlag( const DString & strKey,
                        unsigned long ulAddFlag );
    
    /**
     * This method performs like the getObject method.
     * The only difference is that this method allows
     * you to get a CBO pointer even if the current
     * instance is marked for no const pointer access.
     * 
     * 
     * Retrieve a pointer to the CBOBase object
     * found at the passed in iterator position in
     ...
     * This method finds the iterator base on the passed-in
     * string key value and then calls the overloaded
     * authorizedGetObject method that takes an iterator.
     * 
     * @param strKey A unique string key value used for searching
     *               the collection of CBO/Data objects. This key
     *               should be in the format of an IDI string, example:
     *               Fund_Code=0000001;Account_Number=00000054961;
     * @return A pointer to a CBOBase object.
     * @see authorizedSetObject
     * @see setObject
     * @see deleteObject
     */
    CBOBase * authorizedGetObject(const DString &strKey);
     ...
     * @see deleteObject
     */
    CBOBase * authorizedGetObject(UPDLIST_ITER x);

    /**
     * This method performs like setObject method.
     * The only difference is that this method
     * allows you to specify the ACTIVITY flag.
     * 
     * Sets the provided CBOBase derived object pointer
     * into the CBO/Data object list.  This method
     * uses the string key to find the matching before
     ...
     * the CBO pointer if he owns it.
     * 
     * The corresponding ACTIVITY flag will be set to
     * the specifie ACTIVITY value.
     * 
     * This method will also increment the updatelevel value
     * as a notification mechanism.
     * 
     ...
     * @see listHasBeenModified
     */
    SEVERITY authorizedSetObject( const DString & strKey,
                                  CBOBase * pCBOBase,
                              const bool fListOwnsObject = false,
          updListItem::ACTIVITY updActivity = updListItem::NONE);

    /**
     * Removes all CBOs and Data objects and any corresponding
     * ACTIVITY flags and miscellaneous flags.
     * 
     * @return true if successful
     *         false if failure
     */
     ...
     * versions of UpdatableListBaseImpl.
     * 
     * Originally this was supposed to be called
     * from the constructCBOForAllDataItems method
     * 
     * This method should be overridden
     * and implemented in the derived class.
     * 
     * The implementation should construct
     * a CBO from the passed in Data object
     * and give it
     * 
     * @param data   The Data object to be used for instantiating a
     *               CBO.
     * @param strKey A string variable to be filled with unique
     *               string key value used for searching
     *               the collection of CBO/Data objects. This key
     *               should be in the format of an IDI string, example:
     *               Fund_Code=0000001;Account_Number=00000054961;
     * @return A CBOBase derived object pointer
     * @see constructCBOForAllDataItems
     */
    virtual CBOBase * constructCBOFromDataObject( 
                                               const Data & data,
                                               DString & strKey);

    /**
     * Gets a miscellaneous bit flag value for an item in the
     * CBO/Data object list.
     * 
     * @param strKey A unique string key value used for searching
     *               the collection of CBO/Data objects. This key
     ...
     *               Fund_Code=0000001;Account_Number=00000054961;
     * @param ulObjectFlag
     *               a long value to be set as a bit flag
     * @return true if successful
     *         false if failure
     * @see addObjectFlag
     * @see isObjectFlagSet
     * @see removeObjectFlag
     */
    bool getObjectFlag( const DString & strKey,
                        unsigned long & ulObjectFlag );
    
    /**
     * Increment the long value of the update level by one.
     * 
     * Increase the update level associated with an
     * instance of an UpdatableListBaseImpl derived object.
     * 
     * Use this method as a means of notification for
     * changes that have occurred on the current
     ...
     * @see removeObjectFlag
     */
    void incrementUpdateLevel();

    /**
     * Determines if a miscellaneous bit flag value for an item in the
     * CBO/Data object list is on.
     * 
     * @param strKey A unique string key value used for searching
     *               the collection of CBO/Data objects. This key
     *               should be in the format of an IDI string, example:
     *               Fund_Code=0000001;Account_Number=00000054961;
     * @param ulObjectFlag
     *               a long value as a bit flag to check
     *               for on or off
     * @return true if the object flag is on
     *         false if the object flag is off
     * @see addObjectFlag
     * @see getObjectFlag
     * @see removeObjectFlag
     */
    bool isObjectFlagSet( const DString & strKey,
                          unsigned long ulObjectFlag );

    /**
     * This method can be overridden and implemented
     * in a derived class.  UpdatableListBaseImpl uses
     * this method as a way to notify the derived
     * object that the object state of the list has
     * changed.
     * 
     * The methods that will send this notification are:
     * 
     * setObject
     * deleteObject
     * removeObjectAndActivity
     ...
     * @see authorizedSetObject
     */
    virtual void listHasBeenModified() const;

    /** Assignment operator */
    UpdatableListBaseImpl & operator=( 
                            const UpdatableListBaseImpl & copy );
    
    /**
     * Clears a miscellaneous bit flag value for an item in the
     * CBO/Data object list.
     * 
     * @param strKey A unique string key value used for searching
     *               the collection of CBO/Data objects. This key
     ...
     *               Fund_Code=0000001;Account_Number=00000054961;
     * @param ulRemoveFlag
     *               a long value to be cleared as a bit flag
     * @return 
     * @see addObjectFlag
     * @see getObjectFlag
     * @see isObjectFlagSet
     */
    bool removeObjectFlag( const DString & strKey,
                           unsigned long ulRemoveFlag );
    
    /** Copy Constructor
     * 
     * @param copy
     */
    UpdatableListBaseImpl( const UpdatableListBaseImpl &copy );
   ...
   private: //data
    ...
    long lUpdateLevel;
    bool fConstPointerAccess;
    bool fFullCBOObjects;
    int nNextKey;
};
...
//
//
// $Log:   Y:/VCS/iFastAWD/inc/updatablelistbaseimpl.hpp-arc  $
//
//   Rev 1.0   28 Jan 2008 13:33:14   kovacsro
//Initial revision.
// 
//    Rev 1.15   28 Nov 2001 12:42:22   dt14177
// fixed linkage define
// 
//    Rev 1.14   May 23 2001 17:43:06   DT39990
// Added getContainer() method
// 
//    Rev 1.13   Dec 27 2000 16:11:48   dt39990
// Added more documentation and
// organized methods in alphabetical listing
// 
//    Rev 1.12   29 Nov 2000 10:24:20   dt14177
// cleanup
// 
//    Rev 1.11   Aug 23 2000 08:54:22   DT29758
// added method makeCopy()  for derived classes
// to use as a deep copy constructor
// 
//    Rev 1.10   May 05 2000 11:34:28   DT29758
// fixed rollback memory deletion 
// 
//    Rev 1.9   May 02 2000 12:03:28   DT29758
...
//     doesCheckpointExist
// 
//    Rev 1.8   Mar 02 2000 11:32:26   DT29758
// Added methods for counting number of
// objects with activity set to a type
// 
//    Rev 1.5   Dec 31 1999 06:27:54   DT29758
// Minor Edits to Java-Doc comments
// 
//    Rev 1.3   Dec 06 1999 10:21:44   DT29758
// added ccdoc code tag directives for using 
// namespaces
// 
//    Rev 1.2   Dec 02 1999 16:03:50   DT29758
...
// 
//    Rev 1.0   Jun 21 1999 17:16:06   DT22388
//  
// 
//    Rev 1.4   Jan 28 1999 14:43:36   DT22388
...
// 
// 
//    Rev 1.3   Jan 14 1999 12:49:36   DT21815
// Changed #include file names to lower case.
// 
//    Rev 1.2   Oct 31 1998 10:11:14   DJOO
// Added virtual keyword to 2 getObject methods.
// 
//    Rev 1.1   Jul 30 1998 14:17:12   djoo
// Added methods removeAllObjectsAndActivity, 
// isObjectInList and isObjectOwned.
// 
#endif // UPDATABLELISTBASEIMPL_HPP
