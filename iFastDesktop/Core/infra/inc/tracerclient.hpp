...
#endif
#pragma message("tracerclient.hpp")
...
#if defined(EXECHIST_TRACING)
#include <bfutil/exechist.hpp>
#include <bf/log/log.hpp>

void inline logClass( const I_CHAR* pszClass ) {
   bf::log::LogStream ls( bf::log::awd );
   ls << bf::log::information;
   ls << pszClass << bf::log::endlf;
}
void inline logClassMethod( const I_CHAR* pszClass, const I_CHAR* pszMethod ) {
   bf::log::LogStream ls( bf::log::awd );
   ls << bf::log::information;
   ls << pszClass << I_("::") << pszMethod << bf::log::endlf;
}
void inline logMessage( const I_CHAR* pszMessage ) {
   bf::log::LogStream ls( bf::log::awd );
   ls << bf::log::information;
   ls << pszMessage << bf::log::endlf;
}
void inline logClassMessage( const I_CHAR* pszClass, const I_CHAR* pszMethod, const I_CHAR* pszMessage ) {
   bf::log::LogStream ls( bf::log::awd );
   ls << bf::log::information;
   ls << pszClass << I_("::") << pszMethod << I_(" ") << pszMessage << bf::log::endlf;
}
...
#define TRACE_CONSTRUCTOR( classname, signature ) \
   { \
      EXECHIST2( classname, signature ); \
      logClassMethod( classname, signature ); \
      TracerClient tracerClient; \
      if (tracerClient.isTracing() ) \
      { \
...
#define TRACE_DESTRUCTOR( classname ) \
   { \
      EXECHIST2( classname, I_("dtor") ); \
      logClassMethod( classname, I_("dtor") ); \
      TracerClient tracerClient(classname); \
   }

#define TRACE_METHOD( classname, methodsignature ) \
   { \
      EXECHIST2( classname, methodsignature ); \
      logClassMethod( classname, methodsignature ); \
      TracerClient tracerClient(classname); \
      if ( tracerClient.isTracing() ) \
      { \
         tracerClient.setSupportOldTracing( true); \
...
//
//
// $Log:   Y:/VCS/iFastAWD/inc/tracerclient.hpp-arc  $
//
//   Rev 1.0   28 Jan 2008 13:33:10   kovacsro
//Initial revision.
// 
