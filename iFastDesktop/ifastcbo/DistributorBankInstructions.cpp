...
//*****************************************************************************
//
//    ^FILE   : DistributorBankInstructions.cpp
...
//-----------------------------------------------------------------------------
//
// ^CLASS  : DistributorBankInstructions
//
// ^MEMBER VARIABLES :
//
// ^CODING COMMENTS :
...
namespace 
{
   const I_CHAR * CLASSNAME = I_( "DistributorBankInstructions" );
}
...
const BANKFIELDMAPPINGINFO distributor_bankFieldMappingInfo[] = {
...
int distributor_bankFieldMappingInfo_size = 
   sizeof(distributor_bankFieldMappingInfo) / sizeof(_bankFieldMappingInfo);

//******************************************************************************
DistributorBankInstructions::DistributorBankInstructions( BFAbstractCBO &parent, 
                                                          const DString &bankType): 
      BankInstructionsEx( parent, 
                          bankType, 
                          distributor_bankFieldMappingInfo, 
                          distributor_bankFieldMappingInfo_size)
{
   TRACE_CONSTRUCTOR(CLASSNAME , NULL_STRING);
}

//******************************************************************************
DistributorBankInstructions::~DistributorBankInstructions()
{
   TRACE_DESTRUCTOR( CLASSNAME );
}

//******************************************************************************
SEVERITY DistributorBankInstructions::initNew (const BFDataGroupId &idDataGroup)
{
...
//******************************************************************************
BFAbstractCBO *DistributorBankInstructions::getParentCBO ()
{
   assert (getParent ());
   BFAbstractCBO * parentCBO = 
      ((dynamic_cast <DistributorBankInstructionsList*> (getParent ()))->getPayInstructionsParentCBO ());
   
   return parentCBO ? parentCBO : getParent ()->getParent ();
}

//******************************************************************************
SEVERITY DistributorBankInstructions::getParentProcessDate ( DString &processDate, 
...
//******************************************************************************
SEVERITY DistributorBankInstructions::getParentCurrency ( DString &currency,
                                                          DString &description,
                                                          const BFDataGroupId &idDataGroup)
{
...
//******************************************************************************
SEVERITY DistributorBankInstructions::doInitBeforeCloning (const BFDataGroupId &idDataGroup)
{
...
//******************************************************************************
SEVERITY DistributorBankInstructions::preInit (const BFDataGroupId &idDataGroup)
{
   ...
   pDistributorBankInstructionsList = dynamic_cast <DistributorBankInstructionsList*>(getParent ());
   if(pDistributorBankInstructionsList)
   {
	   PaymentInstructions* pPaymentInstructions = dynamic_cast <PaymentInstructions*>(pDistributorBankInstructionsList->getPayInstructionsParentCBO());
	   if(pPaymentInstructions)
	   {
		   pPaymentInstructions->getField(ifds::PayMethod, payMethod, idDataGroup, false);
		   pPaymentInstructions->getField(ifds::ACHProcessor, fileProcessor, idDataGroup, false);
   ...
   if (transType == PAYMENT_FOR::TRAILER_FEE)
   {
      setFieldNoValidate (ifds::AcctUseCode, AcctUseCode::TRAILER_FEE, idDataGroup);
   }
   else if (transType == PAYMENT_FOR::COMMISSION)
   {
      setFieldNoValidate (ifds::AcctUseCode, AcctUseCode::COMMISSION, idDataGroup);
   }
   else if (transType == PAYMENT_FOR::DISTRIBUTION)
   {
      setFieldNoValidate (ifds::AcctUseCode, AcctUseCode::EFT_DIVIDEND, idDataGroup);
   }
   else if (transType == PAYMENT_FOR::REDEMPTION)
   {
...
//******************************************************************************
SEVERITY DistributorBankInstructions::postInit (const BFDataGroupId &idDataGroup)
{
...
//******************************************************************************
//
/* $Log:   Y:/VCS/BF iFAST/ifastcbo/DistributorBankInstructions.cpp-arc  $
// 
//    Rev 1.14   Sep 21 2006 19:28:20   popescu
// STP PET 2213/17
// 
//    Rev 1.13   Sep 13 2006 19:10:52   popescu
// STP 2192/FN 13
// 
//    Rev 1.12   Sep 12 2006 10:52:12   popescu
// STP - PET 2192/11
// 
