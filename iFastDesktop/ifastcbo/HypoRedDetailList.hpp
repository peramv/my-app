#pragma once

//******************************************************************************
//
// COPYRIGHT:
//
//    The computer systems, procedures, data bases and
//    programs created and maintained by DST Systems, Inc.,
//    are proprietary in nature and as such are confidential.
//    Any unauthorized use or disclosure of such information
//    may result in civil liabilities.
//
//    Copyright 2000 by DST Systems, Inc.
//
//
//******************************************************************************
//
// ^FILE   : HypoRedDetailList.hpp
// ^AUTHOR : Robert Kovacs
// ^DATE   : 10/29/2002
//
// ^CLASS    : HypoRedDetailList
// ^SUBCLASS :
//
// ^CLASS DESCRIPTION :
//
//******************************************************************************

#include <ifastcbo\mfcancbo.hpp>
#include <boost\utility.hpp>

#undef IFASTCBO_LINKAGE
#ifdef IFASTCBO_DLL
   #define IFASTCBO_LINKAGE CLASS_EXPORT
#else
   #define IFASTCBO_LINKAGE CLASS_IMPORT
#endif

class HypoRedDetailList;

class IFASTCBO_LINKAGE HypoRedDetailList : public MFCanBFCbo, private boost::noncopyable
{
public:

   HypoRedDetailList( BFAbstractCBO &parent );
   virtual ~HypoRedDetailList();

   SEVERITY init( const DString& strFundCode, const DString& strClassCode, 
		const BFData& allDetailsData, const DString& dstrTrack = I_("N"),
      const DString& dstrPageName = NULL_STRING );


protected:

   /**
    * virtual function. It should be overwriten in order to do special processing on object creation
    * ( in lists when we are adding an existing element )
    * @param data    - the data object from which we are taking data from
    * @param pObjOut - here we are returning the new object
 
   */
   SEVERITY doCreateObject( const BFData& data, BFCBO *&pObjOut );
	

private:



};

//******************************************************************************
//              Revision Control Entries
//******************************************************************************
/* $Log:   Y:/VCS/BF iFAST/ifastcbo/HypoRedDetailList.hpp-arc  $
//
//   Rev 1.3   Nov 14 2004 14:44:42   purdyech
//PET910 - .NET Conversion
//
//   Rev 1.2   Mar 21 2003 18:14:24   PURDYECH
//BFDataGroupId/BFContainerId changeover
//
//   Rev 1.1   Nov 25 2002 09:41:20   KOVACSRO
//Added implementation.
//
//   Rev 1.0   Oct 29 2002 10:51:38   KOVACSRO
//Initial Revision
// 
 
*/

