// BFDataImpl implementation file for DataSet DSTC0305.REQ
// TIMBUK3: This file was generated by Timbuk3 - Version 2.4.1
// TIMBUK3: Generated on : Mon Aug 24 15:59:59 2015


#include "stdafx.h"




#include <bfdata/bfdataimpl.hpp>
#include "dsi_dstc0305_req.h"


// This function creates an instance of dsi_DSTC0305_REQ
// The function is required by the factory class as that class may choke the compiler
// with excessive symbols if all of the required header files needed to be #include'd
BFDataImpl* create_dsi_DSTC0305_REQ() { return new dsi_DSTC0305_REQ; }

dsi_DSTC0305_REQ::dsi_DSTC0305_REQ()
: BFDataImpl( 40004624 )
, SessionId_( ifds::SessionId, &ifds::s_FldProp_SessionId )
, User_Id_( ifds::User_Id, &ifds::s_FldProp_User_Id )
, LanguageCode_( ifds::LanguageCode, &ifds::s_FldProp_LanguageCode )
, CompanyId_( ifds::CompanyId, &ifds::s_FldProp_CompanyId )
, Track_( ifds::Track, &ifds::s_FldProp_Track )
, Activity_( ifds::Activity, &ifds::s_FldProp_Activity )
, OverrideRepeats_( ifds::OverrideRepeats, &ifds::s_FldProp_OverrideRepeats )
, NextKey_( ifds::NextKey, &ifds::s_FldProp_NextKey )
, FeeModelCode_( ifds::FeeModelCode, &ifds::s_FldProp_FeeModelCode )
, FeeModelDesc_( ifds::FeeModelDesc, &ifds::s_FldProp_FeeModelDesc )
, HoldingCompCode_( ifds::HoldingCompCode, &ifds::s_FldProp_HoldingCompCode )
, HoldingCompName_( ifds::HoldingCompName, &ifds::s_FldProp_HoldingCompName )
, AsofDate_( ifds::AsofDate, &ifds::s_FldProp_AsofDate )
, SearchAll_( ifds::SearchAll, &ifds::s_FldProp_SearchAll )
{
   cFields_ = 14;
   memset( &aFlds_[0], 0x00, sizeof( aFlds_ ) );
}

dsi_DSTC0305_REQ::~dsi_DSTC0305_REQ()
{
}

BFDataImpl* dsi_DSTC0305_REQ::clone()
{
   dsi_DSTC0305_REQ* p = new dsi_DSTC0305_REQ;
   p->SessionId_ = SessionId_;
   p->User_Id_ = User_Id_;
   p->LanguageCode_ = LanguageCode_;
   p->CompanyId_ = CompanyId_;
   p->Track_ = Track_;
   p->Activity_ = Activity_;
   p->OverrideRepeats_ = OverrideRepeats_;
   p->NextKey_ = NextKey_;
   p->FeeModelCode_ = FeeModelCode_;
   p->FeeModelDesc_ = FeeModelDesc_;
   p->HoldingCompCode_ = HoldingCompCode_;
   p->HoldingCompName_ = HoldingCompName_;
   p->AsofDate_ = AsofDate_;
   p->SearchAll_ = SearchAll_;
   return(p);
}

BFDataField* dsi_DSTC0305_REQ::getElement( const BFFieldId& id )
{
   switch( id.getId() ) {
      case 40000005: return ( &SessionId_ ); break; // SessionId
      case 40000002: return ( &User_Id_ ); break; // User_Id
      case 40000030: return ( &LanguageCode_ ); break; // LanguageCode
      case 40000031: return ( &CompanyId_ ); break; // CompanyId
      case 40002065: return ( &Track_ ); break; // Track
      case 40002066: return ( &Activity_ ); break; // Activity
      case 40002139: return ( &OverrideRepeats_ ); break; // OverrideRepeats
      case 40000029: return ( &NextKey_ ); break; // NextKey
      case 40004584: return ( &FeeModelCode_ ); break; // FeeModelCode
      case 40004618: return ( &FeeModelDesc_ ); break; // FeeModelDesc
      case 40007044: return ( &HoldingCompCode_ ); break; // HoldingCompCode
      case 40007043: return ( &HoldingCompName_ ); break; // HoldingCompName
      case 40000061: return ( &AsofDate_ ); break; // AsofDate
      case 40007047: return ( &SearchAll_ ); break; // SearchAll
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0305_REQ::getElement( const BFFieldId& id ) const
{
   return( ( const_cast< dsi_DSTC0305_REQ* >(this) )->getElement( id ) );
}

BFDataField* dsi_DSTC0305_REQ::getElementByIndex( unsigned int iField )
{
   if ( iField >=0 && iField < 14 )
   {
      BFDataField* pField = aFlds_[iField];
      if( NULL == pField )
      {
         switch(iField)
         {
            case 0: aFlds_[0] = &SessionId_; break;
            case 1: aFlds_[1] = &User_Id_; break;
            case 2: aFlds_[2] = &LanguageCode_; break;
            case 3: aFlds_[3] = &CompanyId_; break;
            case 4: aFlds_[4] = &Track_; break;
            case 5: aFlds_[5] = &Activity_; break;
            case 6: aFlds_[6] = &OverrideRepeats_; break;
            case 7: aFlds_[7] = &NextKey_; break;
            case 8: aFlds_[8] = &FeeModelCode_; break;
            case 9: aFlds_[9] = &FeeModelDesc_; break;
            case 10: aFlds_[10] = &HoldingCompCode_; break;
            case 11: aFlds_[11] = &HoldingCompName_; break;
            case 12: aFlds_[12] = &AsofDate_; break;
            case 13: aFlds_[13] = &SearchAll_; break;
         }
         pField = aFlds_[iField];
         if( NULL == pField ) return( NULL );
      }
      return( pField );
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0305_REQ::getElementByIndex( unsigned int iField ) const
{
   return( ( const_cast< dsi_DSTC0305_REQ* >(this) )->getElementByIndex( iField ) );
}

unsigned long dsi_DSTC0305_REQ::getLargestDefinedField( void ) const
{
   return( 100 );
}

unsigned long dsi_DSTC0305_REQ::getDefinedByteLength( void ) const
{
   return( 671 * sizeof( I_CHAR ) );
}

bool dsi_DSTC0305_REQ::exists( const BFFieldId& id ) const
{
   return dsi_DSTC0305_REQ::fieldExists( id );
}

bool dsi_DSTC0305_REQ::fieldExists( const BFFieldId& id )
{
   switch( id.getId() )
   {
      case 40000005: return( true );
      case 40000002: return( true );
      case 40000030: return( true );
      case 40000031: return( true );
      case 40002065: return( true );
      case 40002066: return( true );
      case 40002139: return( true );
      case 40000029: return( true );
      case 40004584: return( true );
      case 40004618: return( true );
      case 40007044: return( true );
      case 40007043: return( true );
      case 40000061: return( true );
      case 40007047: return( true );
   }
   return false;
}

