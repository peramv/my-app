// BFDataImpl implementation file for DataSet DSTC0056.REQ
// TIMBUK3: This file was generated by Timbuk3 - Version 2.4.1
// TIMBUK3: Generated on : Mon Aug 24 15:59:58 2015


#include "stdafx.h"




#include <bfdata/bfdataimpl.hpp>
#include "dsi_dstc0056_req.h"


// This function creates an instance of dsi_DSTC0056_REQ
// The function is required by the factory class as that class may choke the compiler
// with excessive symbols if all of the required header files needed to be #include'd
BFDataImpl* create_dsi_DSTC0056_REQ() { return new dsi_DSTC0056_REQ; }

dsi_DSTC0056_REQ::dsi_DSTC0056_REQ()
: BFDataImpl( 40000099 )
, NextKey_( ifds::NextKey, &ifds::s_FldProp_NextKey )
, SessionId_( ifds::SessionId, &ifds::s_FldProp_SessionId )
, User_Id_( ifds::User_Id, &ifds::s_FldProp_User_Id )
, LanguageCode_( ifds::LanguageCode, &ifds::s_FldProp_LanguageCode )
, CompanyId_( ifds::CompanyId, &ifds::s_FldProp_CompanyId )
, AllocEffectiveDate_( ifds::AllocEffectiveDate, &ifds::s_FldProp_AllocEffectiveDate )
, Track_( ifds::Track, &ifds::s_FldProp_Track )
, Activity_( ifds::Activity, &ifds::s_FldProp_Activity )
, OverrideRepeats_( ifds::OverrideRepeats, &ifds::s_FldProp_OverrideRepeats )
, SearchFundType_( ifds::SearchFundType, &ifds::s_FldProp_SearchFundType )
, FundWKN_( ifds::FundWKN, &ifds::s_FldProp_FundWKN )
, FundISIN_( ifds::FundISIN, &ifds::s_FldProp_FundISIN )
, FundNumber_( ifds::FundNumber, &ifds::s_FldProp_FundNumber )
, FundName_( ifds::FundName, &ifds::s_FldProp_FundName )
, FundCode_( ifds::FundCode, &ifds::s_FldProp_FundCode )
, ClassCode_( ifds::ClassCode, &ifds::s_FldProp_ClassCode )
{
   cFields_ = 16;
   memset( &aFlds_[0], 0x00, sizeof( aFlds_ ) );
}

dsi_DSTC0056_REQ::~dsi_DSTC0056_REQ()
{
}

BFDataImpl* dsi_DSTC0056_REQ::clone()
{
   dsi_DSTC0056_REQ* p = new dsi_DSTC0056_REQ;
   p->NextKey_ = NextKey_;
   p->SessionId_ = SessionId_;
   p->User_Id_ = User_Id_;
   p->LanguageCode_ = LanguageCode_;
   p->CompanyId_ = CompanyId_;
   p->AllocEffectiveDate_ = AllocEffectiveDate_;
   p->Track_ = Track_;
   p->Activity_ = Activity_;
   p->OverrideRepeats_ = OverrideRepeats_;
   p->SearchFundType_ = SearchFundType_;
   p->FundWKN_ = FundWKN_;
   p->FundISIN_ = FundISIN_;
   p->FundNumber_ = FundNumber_;
   p->FundName_ = FundName_;
   p->FundCode_ = FundCode_;
   p->ClassCode_ = ClassCode_;
   return(p);
}

BFDataField* dsi_DSTC0056_REQ::getElement( const BFFieldId& id )
{
   switch( id.getId() ) {
      case 40000029: return ( &NextKey_ ); break; // NextKey
      case 40000005: return ( &SessionId_ ); break; // SessionId
      case 40000002: return ( &User_Id_ ); break; // User_Id
      case 40000030: return ( &LanguageCode_ ); break; // LanguageCode
      case 40000031: return ( &CompanyId_ ); break; // CompanyId
      case 40002074: return ( &AllocEffectiveDate_ ); break; // AllocEffectiveDate
      case 40002065: return ( &Track_ ); break; // Track
      case 40002066: return ( &Activity_ ); break; // Activity
      case 40002139: return ( &OverrideRepeats_ ); break; // OverrideRepeats
      case 40004673: return ( &SearchFundType_ ); break; // SearchFundType
      case 40004586: return ( &FundWKN_ ); break; // FundWKN
      case 40004587: return ( &FundISIN_ ); break; // FundISIN
      case 40002893: return ( &FundNumber_ ); break; // FundNumber
      case 40001841: return ( &FundName_ ); break; // FundName
      case 40000049: return ( &FundCode_ ); break; // FundCode
      case 40000050: return ( &ClassCode_ ); break; // ClassCode
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0056_REQ::getElement( const BFFieldId& id ) const
{
   return( ( const_cast< dsi_DSTC0056_REQ* >(this) )->getElement( id ) );
}

BFDataField* dsi_DSTC0056_REQ::getElementByIndex( unsigned int iField )
{
   if ( iField >=0 && iField < 16 )
   {
      BFDataField* pField = aFlds_[iField];
      if( NULL == pField )
      {
         switch(iField)
         {
            case 0: aFlds_[0] = &NextKey_; break;
            case 1: aFlds_[1] = &SessionId_; break;
            case 2: aFlds_[2] = &User_Id_; break;
            case 3: aFlds_[3] = &LanguageCode_; break;
            case 4: aFlds_[4] = &CompanyId_; break;
            case 5: aFlds_[5] = &AllocEffectiveDate_; break;
            case 6: aFlds_[6] = &Track_; break;
            case 7: aFlds_[7] = &Activity_; break;
            case 8: aFlds_[8] = &OverrideRepeats_; break;
            case 9: aFlds_[9] = &SearchFundType_; break;
            case 10: aFlds_[10] = &FundWKN_; break;
            case 11: aFlds_[11] = &FundISIN_; break;
            case 12: aFlds_[12] = &FundNumber_; break;
            case 13: aFlds_[13] = &FundName_; break;
            case 14: aFlds_[14] = &FundCode_; break;
            case 15: aFlds_[15] = &ClassCode_; break;
         }
         pField = aFlds_[iField];
         if( NULL == pField ) return( NULL );
      }
      return( pField );
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0056_REQ::getElementByIndex( unsigned int iField ) const
{
   return( ( const_cast< dsi_DSTC0056_REQ* >(this) )->getElementByIndex( iField ) );
}

unsigned long dsi_DSTC0056_REQ::getLargestDefinedField( void ) const
{
   return( 200 );
}

unsigned long dsi_DSTC0056_REQ::getDefinedByteLength( void ) const
{
   return( 986 * sizeof( I_CHAR ) );
}

bool dsi_DSTC0056_REQ::exists( const BFFieldId& id ) const
{
   return dsi_DSTC0056_REQ::fieldExists( id );
}

bool dsi_DSTC0056_REQ::fieldExists( const BFFieldId& id )
{
   switch( id.getId() )
   {
      case 40000029: return( true );
      case 40000005: return( true );
      case 40000002: return( true );
      case 40000030: return( true );
      case 40000031: return( true );
      case 40002074: return( true );
      case 40002065: return( true );
      case 40002066: return( true );
      case 40002139: return( true );
      case 40004673: return( true );
      case 40004586: return( true );
      case 40004587: return( true );
      case 40002893: return( true );
      case 40001841: return( true );
      case 40000049: return( true );
      case 40000050: return( true );
   }
   return false;
}

