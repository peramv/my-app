// BFDataImpl implementation file for DataSet DSTC0377.REQ
// TIMBUK3: This file was generated by Timbuk3 - Version 2.4.1
// TIMBUK3: Generated on : Mon Aug 24 15:59:59 2015


#include "stdafx.h"




#include <bfdata/bfdataimpl.hpp>
#include "dsi_dstc0377_req.h"


// This function creates an instance of dsi_DSTC0377_REQ
// The function is required by the factory class as that class may choke the compiler
// with excessive symbols if all of the required header files needed to be #include'd
BFDataImpl* create_dsi_DSTC0377_REQ() { return new dsi_DSTC0377_REQ; }

dsi_DSTC0377_REQ::dsi_DSTC0377_REQ()
: BFDataImpl( 40005558 )
, SessionId_( ifds::SessionId, &ifds::s_FldProp_SessionId )
, User_Id_( ifds::User_Id, &ifds::s_FldProp_User_Id )
, LanguageCode_( ifds::LanguageCode, &ifds::s_FldProp_LanguageCode )
, CompanyId_( ifds::CompanyId, &ifds::s_FldProp_CompanyId )
, Track_( ifds::Track, &ifds::s_FldProp_Track )
, Activity_( ifds::Activity, &ifds::s_FldProp_Activity )
, AccountNum_( ifds::AccountNum, &ifds::s_FldProp_AccountNum )
, BrokerCode_( ifds::BrokerCode, &ifds::s_FldProp_BrokerCode )
, BranchCode_( ifds::BranchCode, &ifds::s_FldProp_BranchCode )
, SlsrepCode_( ifds::SlsrepCode, &ifds::s_FldProp_SlsrepCode )
, AcctDesignation_( ifds::AcctDesignation, &ifds::s_FldProp_AcctDesignation )
, TaxType_( ifds::TaxType, &ifds::s_FldProp_TaxType )
, EffectiveDate_( ifds::EffectiveDate, &ifds::s_FldProp_EffectiveDate )
, CreateRep_( ifds::CreateRep, &ifds::s_FldProp_CreateRep )
, InterCode_( ifds::InterCode, &ifds::s_FldProp_InterCode )
, InterAccount_( ifds::InterAccount, &ifds::s_FldProp_InterAccount )
, AltAccount_( ifds::AltAccount, &ifds::s_FldProp_AltAccount )
{
   cFields_ = 17;
   memset( &aFlds_[0], 0x00, sizeof( aFlds_ ) );
}

dsi_DSTC0377_REQ::~dsi_DSTC0377_REQ()
{
}

BFDataImpl* dsi_DSTC0377_REQ::clone()
{
   dsi_DSTC0377_REQ* p = new dsi_DSTC0377_REQ;
   p->SessionId_ = SessionId_;
   p->User_Id_ = User_Id_;
   p->LanguageCode_ = LanguageCode_;
   p->CompanyId_ = CompanyId_;
   p->Track_ = Track_;
   p->Activity_ = Activity_;
   p->AccountNum_ = AccountNum_;
   p->BrokerCode_ = BrokerCode_;
   p->BranchCode_ = BranchCode_;
   p->SlsrepCode_ = SlsrepCode_;
   p->AcctDesignation_ = AcctDesignation_;
   p->TaxType_ = TaxType_;
   p->EffectiveDate_ = EffectiveDate_;
   p->CreateRep_ = CreateRep_;
   p->InterCode_ = InterCode_;
   p->InterAccount_ = InterAccount_;
   p->AltAccount_ = AltAccount_;
   return(p);
}

BFDataField* dsi_DSTC0377_REQ::getElement( const BFFieldId& id )
{
   switch( id.getId() ) {
      case 40000005: return ( &SessionId_ ); break; // SessionId
      case 40000002: return ( &User_Id_ ); break; // User_Id
      case 40000030: return ( &LanguageCode_ ); break; // LanguageCode
      case 40000031: return ( &CompanyId_ ); break; // CompanyId
      case 40002065: return ( &Track_ ); break; // Track
      case 40002066: return ( &Activity_ ); break; // Activity
      case 40000040: return ( &AccountNum_ ); break; // AccountNum
      case 40000044: return ( &BrokerCode_ ); break; // BrokerCode
      case 40000045: return ( &BranchCode_ ); break; // BranchCode
      case 40000656: return ( &SlsrepCode_ ); break; // SlsrepCode
      case 40000178: return ( &AcctDesignation_ ); break; // AcctDesignation
      case 40000134: return ( &TaxType_ ); break; // TaxType
      case 40000327: return ( &EffectiveDate_ ); break; // EffectiveDate
      case 40001006: return ( &CreateRep_ ); break; // CreateRep
      case 40000186: return ( &InterCode_ ); break; // InterCode
      case 40000187: return ( &InterAccount_ ); break; // InterAccount
      case 40000173: return ( &AltAccount_ ); break; // AltAccount
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0377_REQ::getElement( const BFFieldId& id ) const
{
   return( ( const_cast< dsi_DSTC0377_REQ* >(this) )->getElement( id ) );
}

BFDataField* dsi_DSTC0377_REQ::getElementByIndex( unsigned int iField )
{
   if ( iField >=0 && iField < 17 )
   {
      BFDataField* pField = aFlds_[iField];
      if( NULL == pField )
      {
         switch(iField)
         {
            case 0: aFlds_[0] = &SessionId_; break;
            case 1: aFlds_[1] = &User_Id_; break;
            case 2: aFlds_[2] = &LanguageCode_; break;
            case 3: aFlds_[3] = &CompanyId_; break;
            case 4: aFlds_[4] = &Track_; break;
            case 5: aFlds_[5] = &Activity_; break;
            case 6: aFlds_[6] = &AccountNum_; break;
            case 7: aFlds_[7] = &BrokerCode_; break;
            case 8: aFlds_[8] = &BranchCode_; break;
            case 9: aFlds_[9] = &SlsrepCode_; break;
            case 10: aFlds_[10] = &AcctDesignation_; break;
            case 11: aFlds_[11] = &TaxType_; break;
            case 12: aFlds_[12] = &EffectiveDate_; break;
            case 13: aFlds_[13] = &CreateRep_; break;
            case 14: aFlds_[14] = &InterCode_; break;
            case 15: aFlds_[15] = &InterAccount_; break;
            case 16: aFlds_[16] = &AltAccount_; break;
         }
         pField = aFlds_[iField];
         if( NULL == pField ) return( NULL );
      }
      return( pField );
   }
   return( NULL );
}

const BFDataField* dsi_DSTC0377_REQ::getElementByIndex( unsigned int iField ) const
{
   return( ( const_cast< dsi_DSTC0377_REQ* >(this) )->getElementByIndex( iField ) );
}

unsigned long dsi_DSTC0377_REQ::getLargestDefinedField( void ) const
{
   return( 60 );
}

unsigned long dsi_DSTC0377_REQ::getDefinedByteLength( void ) const
{
   return( 809 * sizeof( I_CHAR ) );
}

bool dsi_DSTC0377_REQ::exists( const BFFieldId& id ) const
{
   return dsi_DSTC0377_REQ::fieldExists( id );
}

bool dsi_DSTC0377_REQ::fieldExists( const BFFieldId& id )
{
   switch( id.getId() )
   {
      case 40000005: return( true );
      case 40000002: return( true );
      case 40000030: return( true );
      case 40000031: return( true );
      case 40002065: return( true );
      case 40002066: return( true );
      case 40000040: return( true );
      case 40000044: return( true );
      case 40000045: return( true );
      case 40000656: return( true );
      case 40000178: return( true );
      case 40000134: return( true );
      case 40000327: return( true );
      case 40001006: return( true );
      case 40000186: return( true );
      case 40000187: return( true );
      case 40000173: return( true );
   }
   return false;
}

