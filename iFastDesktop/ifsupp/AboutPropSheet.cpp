...
void AboutPropSheet::Display( const I_CHAR* pszCaption )
{
  ...
  CPropertyPage* pPageMq = nullptr;
  if (ifastdbrkr::getMessageService() == ifastdbrkr::MessageService::ActiveMQ)
  {
    pPageMq = new AboutPropActiveMqPage();
  }
  else
  {
    pPageMq = new AboutPropSonicPage();
  }

  pPageMq->m_psp.dwFlags &= ~PSP_HASHELP;
...
void AboutPropSheet::OnReport() {
   ...
   ss << I_("Computer: ") << getLocalHost().c_str() << std::endl;
   defaultFileName.Format(I_("%s_%s_%s"), getLocalHost().c_str(), pszUserName, LPCTSTR(strTime) );
   ss << I_("Local IP: ") << getLocalIP().c_str() << std::endl;
   logOS( ss );
   ss << std::endl;
   ...
   if( dlg.DoModal() ) {
      ...
      if( reportFile.Open( dlg.GetPathName(), CFile::modeCreate|CFile::modeWrite ) ) {
