...
namespace {
   ...
   // Required specific market\lang support DLLs
   I_CHAR* MARKET_LANGS_NAMES[] =
   {
      I_("canada_eng"),
      I_("canada_fr"),
      I_("lux_eng"),
...
//   SetDlgItemText( IDC_TXT_INFRASTRUCTURE, latestDate );
   ...
   count = sizeof( MARKET_LANGS_NAMES ) / sizeof( MARKET_LANGS_NAMES[0] );
   static CString marketLangExtension = I_(".dll");
   for( int i = 0; i < count; i++ )
   {
      currentDate = LoadModuleInfo( MARKET_LANGS_NAMES[i] + marketLangExtension );
   }
...
void AboutPropModulePage::LoadDependentInfo( CString dependentName )
{
   ...
   if( NULL != instance )
   {
      I_CHAR buffer[MAXBUFSIZE*2];
      ::GetModuleFileName( instance, buffer, MAXBUFSIZE*2 );
...
BOOL AboutPropModulePage::ShowImports( PIMAGE_DOS_HEADER dosHeader )
{
   ...
   try
   {
      ...
      while( TRUE )
      {
         ...
         LoadDependentInfo( A2W(dependFile) );
