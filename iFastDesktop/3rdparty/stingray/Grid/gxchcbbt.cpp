...
void CGXComboBoxButton::Draw(CDC* pDC, BOOL bActive, const CGXStyle& style)
{
	...
	if (lhTheme)
	{
		...
		else if (bDrawInactive && !style.GetEnabled()) iState = CBXS_DISABLED;
	...
	else
	{
		...
		{
			CBitmap& bmp = (bDrawInactive && !style.GetEnabled()) ? GXGetSysData()->m_bmpDownArrowDisabled : GXGetSysData()->m_bmpDownArrow;
