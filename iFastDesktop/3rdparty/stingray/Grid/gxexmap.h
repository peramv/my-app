...
public:
	CGXExcelMapImpl(){};
	virtual ~CGXExcelMapImpl(){};
	
	// will return a handler for the record
	virtual T* LocateHandler(WORD w);
	virtual void Reset();
	
	virtual CGXGridCore* GetCore() const
	{return m_pCore;};
	
	virtual void SetCore(CGXGridCore* pCore)
	{m_pCore = pCore;};

	virtual GXEXCELMAPCREATOR<T>* GetHandlerMapEntries() const
	{
		return _handlerMapEntries;
	};
	
	virtual DWORD GetEntriesNum()
	{
