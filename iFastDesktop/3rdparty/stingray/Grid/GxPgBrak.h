...
// CGXPageBreakPlugin.h : header file
//
...
struct CGXPageLabelInfo
{
	CRect rc;
	CString strText;
	CGXPageLabelInfo(const CRect& rect, CString s)
		: rc(rect), strText(s)
	{
	}
	CGXPageLabelInfo()
...
struct CGXBreakLineInfo
{
	CRect rc;
	BOOL bCustom;
	CGXBreakLineInfo(const CRect& rect, BOOL b = FALSE)
		: rc(rect), bCustom(b)
	{}
	CGXBreakLineInfo()
...
public:
	...
	virtual void DoPrepareDC(CDC* pDC, CPrintInfo* pInfo);
	void SetPageGrid(int nRowCount, int nColCount, BOOL bFitTo = TRUE);
	virtual void CalcPageSize(CDC* pDC);
	//@cmember
	/* switch the page break mode */
	virtual void SetPageBreakMode(BOOL bPageBreakMode);
	//@cmember
	...
	virtual void AdjustRect(LPRECT pRect, int nHitTest);
	//@cmember
	/* Hit test for page break line dragging */
	virtual int HitTestPageBreak(CPoint pt, LPRECT pRect);
	//@cmember
	/* Plug this plugin into the income view class */
	virtual void Plugin(CGXGridCore* pView);
	//@cmember
	/* Central message handle function called by the view this plugin plugged into */
	virtual BOOL HandleMessage(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult);
	virtual ~CGXPageBreakPlugin();
