...
*/
			...
			if (pInfo->m_bPreview)
			{
				if (pProp->GetPageOrder() == 0)
					pInfo->m_nCurPage = m_nCurrentPageRowIndex*(m_awPageFirstCol.GetSize()-1)
							+m_nCurrentPageColIndex + 1;
				else
					pInfo->m_nCurPage = m_nCurrentPageColIndex*(m_awPageFirstRow.GetSize()-1)
							+m_nCurrentPageRowIndex + 1;
			}
		}
	...
	//		{
			...
			for(ROWCOL nRowParse = nLogTopRow; nRowParse <= nLogBottomRow; nRowParse++)
			{
				if(IsChildShown(nRowParse))
				{
					...
					CGXHierarchyGrid<CGXGridWnd>* pChildGrid = dynamic_cast<CGXHierarchyGrid<CGXGridWnd>*>(GetChildAt(nRowParse));
...
void CGXHierarchyGrid<T>::SetPrinting(BOOL b, BOOL bClientToRect /* = FALSE */)
{
	...
	for(ROWCOL i=0; i<=GetLogRowCount(); i++)
	{
		if(IsChildShown(i))
		{
			CGXGridCore* pChildGrid = dynamic_cast<CGXGridCore*>(GetChildAt(i));
			ASSERT(pChildGrid);
...
void CGXHierarchyGrid<T>::GetChildFontMetrics(CDC* pDC)
{
	...
	for(ROWCOL i=0; i<=GetLogRowCount(); i++)
	{
		if(IsChildShown(i))
		{
			CFriendlyGrid* pChildGrid = (CFriendlyGrid*)dynamic_cast<CGXGridCore*>(GetChildAt(i));
			if(pChildGrid)
