///////////////////////////////////////////////////////////////////////////////
// gxdbforg.cpp : Attach CGXDynamicRecordset to a grid column (CGXODBCGrid)
//
...
// used for MFC 3.0 multi-layered Extension DLL
#ifdef _GXDLL
	#undef GX_DATAEXT
...
#pragma code_seg("GX_SEG_GXDBFORG")
#endif
...
void CGXODBCGrid::AttachForeignTable(ROWCOL nCol, CGXDynamicRecordset* pSet, UINT nKeyCol, UINT nDispCol, BOOL bDispAllCols, BOOL bDisplayHeader)
{
	ASSERT(pSet != NULL);
	...
	// Build choice list
	if (pSet && (!pSet->IsOpen() || pSet->m_nFields == 0))
	{
		TRY
		{
			pSet->Open();
	...
	if (pSet->IsEOF() && pSet->IsBOF()) // empty
		return;

	CString strChoiceList;
	pSet->FillChoiceList(strChoiceList, bDisplayHeader);
