...
GX_INLINE  int CGXGridCore::Height_LPtoDP(LONG nHeight)
	{ 
		 const LONG denom = GX_NXYFACTOR / GetFontHeight();
		 return static_cast<int>(nHeight / (denom != 0 ? denom : 1));
	}

GX_INLINE  int CGXGridCore::Width_LPtoDP(LONG nWidth)
	{ 
		const LONG denom = GX_NXYFACTOR / GetFontWidth();
		return static_cast<int>(nWidth / (denom != 0 ? denom : 1));
	}
