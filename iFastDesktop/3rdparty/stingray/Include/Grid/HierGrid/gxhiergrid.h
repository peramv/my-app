...
BOOL CGXHierarchyGrid<T>::DoScroll(int direction, ROWCOL nCell)
{
	...
	if(nTopRow % 2)
	{
	 	if(nTopRow >3 && direction ==GX_DOWN && IsRowHidden(nTopRow+1))
	    nCell = 2;
	}
...
// in portion rcHide.
/*
   Uncommenting this code results in error C2995: 
   'void CGXHierarchyGrid<T>::HideChildGridsInRect(CRect &)' : function template has already been defined
