...
/** 
 * Returns the last error that occurred in the current thread as an
 * Exception obj.  User must release the object when finished.
 */
HOBJ SMQ_API SMQ_getLastException();
 ...
 *        client will acknowledge any messages it receives. This parameter
 *        will be ignored if the session is transacted.
 * @param sessionName provides a name for the session; this is an
...
int SMQ_API QueueConnection_createQueueSession2(HOBJ qConn, jboolean transacted,jint acknowledgeMode, HOBJ sessionName, HOBJ *pQSession);
 ...
 *
 * @param thisObj     The QueueSession object to operate against.
 * @param sessionName The session's name if it has one, otherwise null.
 *
 * @return SMQ_SUCCESS (0) if successful, non-zero if error.
 *         Use SMQ_getLastError() and SMQ_getLastErrorText() to interrogate error details.
 */
#define QueueSession_getSessionName(thisObj, sessionName)     Session_getSessionName(thisObj, sessionName) 
 ...
 *                         mode due to internal error in JMS Provider.
 */
int SMQ_API Session_getSessionName(HOBJ thisObj, HOBJ *sessionName);
...
int SMQ_API TopicConnection_createTopicSession2(HOBJ topicConn, jboolean transacted, jint ackMode, HOBJ sessionName, HOBJ* pTopicSession);
 ...
 *         Use SMQ_getLastError() and SMQ_getLastErrorText() to interrogate error details.
 */
#define TopicSession_getSessionName(thisObj, sessionName)     Session_getSessionName(thisObj, sessionName) 
